...
#define SIZE_BUFFER2  10//tamaño de fifo control de i2c
...
/* FIFO de funciones tipo void f(void) */
struct _FIFO_void_f_void_{//FIFO PARA UNA VARIABLE para un byte
    unsigned char pop;//void (**pop)(void);//apunta a array de pointers to fuction 
    unsigned char head;//void (**head)(void);//indice 0 del buffer
    unsigned char tail;//void (**tail)(void);//ultimo indice del buffer
    unsigned char push;//void (**push)(void);//pointer buffer 2
	void (**fpp)(void);
    unsigned char (*popf)(void (*f)(void),struct _FIFO_void_f_void_ *s);//quitar el prmer elemento de la fifo 
	unsigned char (*appendByte)(void (*f)(void),struct _FIFO_void_f_void_ *s);//agregar byte al buffer de transmision
    //void (*vaciarbuff)(unsigned char *p1,unsigned char *p2,unsigned char size);
    void (*resetFIFO)(struct _FIFO_void_f_void_ *s);
    unsigned char ncount;//cuenta los nodos llenos en la fifo
    unsigned char size; //size de la fifo
};
...
/* byte operacion  bit MSB op= w=0  r=1, lectura/escritura 
 * bit 6, bit a16= indica el bit 16 de la direccion total
 * n0 a n5 es el numero de bit a procesar, maximo 63*/
struct  _IIC_{
	struct _FIFO_1byte_ op;// op,a16,n5,n4 n3,n2,n1,n0 ; operacion w=0
	struct _FIFO_1byte_ addl;
	struct _FIFO_1byte_ addh;
	struct _FIFO_void_f_void_ f;//fifo de funciones
...
struct _CONTROL_IIC_{
    union _Byte11_{
		unsigned char ByteF;
		struct{
			unsigned char A16:1;//bit de direccion bit 16
...
struct _Comando_DDS{//comando DDS para graficar en touch
	union Bytex{
	   ...
	   struct{
		   ...
		   unsigned char AUTOMATIC:1;//automatico Activado si|no Configuracion Automatica de Producto
...
void init_FIFO_General_vfv(struct _FIFO_void_f_void_ *s,void (**fpp)(void),unsigned char size);
void BarraDet_displayUINT_var(unsigned char posx,unsigned char posy,unsigned short int *usint);
