...
extern struct _GLOBAL_CONTEXTO g,*AjustVibr,*DDSpant,*AjParamProd;
...
void DDS_HANDLER(void){//MANEJADOR MAESTRO DEL DDS--------------------------------------------------------
	//const unsigned char x1=200;//posicion de las letras en elDDS
	     ...
	     /* Asignacion de coordenadas de los cursores segun se la primera vez que se entra o se venga de un botonazo de borrado   */
            ...
            DDSpant->usint1=Deteccion.Sensibilidad;//save in case of save var or not
            DDSpant->usint2=Deteccion.Altura;
            DDSpant->igxc6=Deteccion.phase;
            DDSpant->igxc7=Deteccion.phasefrac;
            DDSpant->jgxc=zoom.Zoom;
            DDSpant->kgxc=Deteccion.Ganacia;
            DDSpant->lgxc=Deteccion.cuadro;
...
void DisplayParametrosdeDeteccion(void){
	 ...
	 word n;
		...
		displayCuadroMadre();
		ParamDetMenu.indexchar=&g.igxc0;//idice del char a cambiar
        	
		*ParamDetMenu.indexchar=0;//char num 0 del numero 
		cursorx=POSX0;cursory=POSY4;
	    isEnable_Keypad(WAIT);//Desabilita el teclado uno milisegundos.
