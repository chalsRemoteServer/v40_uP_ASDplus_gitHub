...
unsigned char z[3];
	   	...
	   	if( DDSpant->igxc5>0){//esta seleccionado cambiar por numero
	   			operacionVariable(POSXDDSNUMS-8,DDSpant->igxc5,SUMAR,&Deteccion.Sensibilidad);
	   		    menu.contexto.Modificado=AJUSTE_PARAMETRICO_DE_PRODUCTO;
...
unsigned char z[3];
	   	...
	   	if((!DDSpant->igxc5)&&(!DDSpant->igxc1)){//no estamos manipulando digito x digito
	      	displayControlCursorSubMenusKey(PABAJO,&cursorx,&cursory,POSXESQ235,p,DDS_MENU_X);
	   	    return;}
	   	if(DDSpant->igxc5>0){//esta seleccionado cambiar por numero
	   			operacionVariable(POSXDDSNUMS-8,DDSpant->igxc5,RESTAR,&Deteccion.Sensibilidad);
	   			menu.contexto.Modificado=AJUSTE_PARAMETRICO_DE_PRODUCTO;
				dds.Bnderas.bit.DDS1_BORRAR=FALSE;
				dds.Bnderas.bit.Apagando=TRUE; //se Desactiva el Despligue de los Puntitos en pantalla del DD
				displayControladorPantalla(PANTALLA_DDS);
...
unsigned char z[3];
	    ...
	    switch(cursory){
	    	case POSY0:if(menu.contexto.Modificado==YES){
...
void ParametrosdeDeteccionkeyRT(void){
	if(cursory==POSY2){return; }
	if(cursory==POSY4){if(*ParamDetMenu.indexchar>0)//para que no empalmen los algoritmos
		                    return;
...
unsigned char b[6]={" "};  
 switch(cursory){
	case POSY2:break;
	case POSY4://display_UInt_CharsSelected(Deteccion.Sensibilidad,ParamDetMenu.indexchar,POSY4,POSXPARAMDETECT);
	    	    procSensxDigitoEN(ParamDetMenu.indexchar,POSXPARAMDETECT-8,0,Deteccion.Sensibilidad);
		        break;
	case POSY6: return;
	case POSY8: return;
...
void AjusteProductoModekeyEN(void){//Del Menu Automatic o Manual
	if(cursory==POSY0){
		switch(menu.contexto.Modificado){
			...
			default:break;}
            return;}
    if(cursory==POSY2){//debug Queda pendiente la AUTOMATICO
	   return;}
    if(cursory==POSY4){//Ajuste Manual
      switch(menu.contexto.Anterior){	
