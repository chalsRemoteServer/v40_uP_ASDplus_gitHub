...
unsigned char arg4,arg5,arg6,arg8,arg9,arg10;//argumentos para entrar en un contexto
...
//extern struct ProductDataBaseNode{
	//       unsigned char ID; //ID DEL producto para encontrarlo en la base de datos
	  //     char name[NOMBRESIZEPROD];    //nombre del producto
	    //   unsigned int Sensibilidad;  //sensibilidad 
	      // unsigned int Altura;
	     //  unsigned char phase;
	      // unsigned char phasefrac;
	      // char     zoomDDS;
	      // char     gananciaDDS;
	      // char     saved;   //se han salvado alguno de estos parametros??
//}ProdDBNode;//producto Actual traido de la memoria al init
...
extern struct _GLOBAL_CONTEXTO g,*AjustVibr,*DDSpant,*AjParamProd,*ParamDet,*AutomProd;
extern struct _GLOBAL_CONTEXT0 *SelProd;
...
struct _Menu_  menu;
struct _Productos_ prod;
//struct _Param_Det_Menu_ ParamDetMenu;
struct ArbolMenu MenuActualScreen;//la estrucrura del menu actual en pantalla.
struct _TIMER_ timer0,*timer_Automatic;
...
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++-------- PASO-2							 
		 case PANTALLA_DDS://---------------------------------------------------------- PASO 3 IR A VFDkey.h-----
							 ...
							 break;
		 case AUTOMATICO_PROD://---------------------------------------------------------- PASO 3 IR A VFDkey.h-----
							 MenuActualScreen.menuNodo=AUTOMATICO_PROD;
							 MenuActualScreen.permisos=DESARROLLADOR_PERMISO;
							 MenuActualScreen.func=Display_Automatico_Producto;
							 MenuActualScreen.funcKeyUP=AutomaticProdkeyUP;
							 MenuActualScreen.funcKeyRT=AutomaticProdkeyRT;	 
							 MenuActualScreen.funcKeyLF=AutomaticProdkeyLF;
							 MenuActualScreen.funcKeyDN=AutomaticProdkeyDN;
							 MenuActualScreen.funcKeyEN=AutomaticProdkeyEN;
							 break;		 
...
void cursorMenuControlService(unsigned char key,struct ArbolMenu *Pantalla){//(key,void (*a)())
	   switch(key){case keyUP:Pantalla->funcKeyUP();break;
		           ...
		           default:break;//fin default de segundo switch de PORTAL INICIO
		         }//fin switch second de PORTAL INICIO
...
void DisplayAjusteParamProd(void){
	...
	word n; //MENU AJUSTE PARAMETRICO DE PRODUCTO
	   ...
	   // displayFaseVar();
        ...
        AjParamProd->usint1=Deteccion.Sensibilidad;
        AjParamProd->igxc1=Deteccion.phase;
        AjParamProd->igxc2=Deteccion.phasefrac;
        AjParamProd->igxc3=0;//control de reset de conteo
...
void displayIDcommNUM(void){
  ...
  menu.contexto.Modificado=ID_COMUNICACIONES;//indica que ha sido modificado la configuracnio de entradas de sistema	
...
void Display_Automatico_Producto(void){
//	if(DDSpant!=&g)
//		DDSpant=&g;	
//	if(DDSpant->igxc3!=0xCC)//Ya estamos, pero se reimprime
//	     DDSpant->igxc3=0xBB;//primera vez en automatico
//	timer_Automatic=&timer0;
//	DDS_HANDLER();
	unsigned char a[]=">X";
	unsigned char b[]="Ajuste de Producto";
	unsigned char c[]="Pasar Producto";
	
		
	unsigned char x[3]={236,54,0};//235,200,200,200,200,200,200};
	unsigned char y[3]={  0,  0, POSY2};
word n;	
	 VFDclrscr();// limpiar pantalla
	 Reset_estructuras_Menu(&AutomProd);
	 VFDposicion(x[0],y[0]);delay1us();
	 VFDserial_SendBlock(&a[0],sizeof(a),&n);
	 VFDposicion(x[1],y[1]);delay1us();
	 VFDserial_SendBlock(&b[0],sizeof(b),&n);
	 VFDposicion(x[2],y[2]);delay1us();
	 VFDserial_SendBlock(&c[0],sizeof(c),&n);
	 AutomProd->igxc0=1;//Numero de producto a Pasar
	 
}//fin Display_Automatico_Producto---------------------------------------
...
unsigned char v[]="      ";
unsigned char m[]="Ajuste de Producto:"; 
unsigned char k[]="Pase Producto:";
unsigned char x[8]={236,POSXDDS,POSXDDS,POSXDDS,POSXDDS,POSXDDS,POSXDDS,POSXDDS};//235,200,200,200,200,200,200};
unsigned char y[8]={  0,  2,  4,  6,  8, 10, 12, 14};
const unsigned char h[5]={248,240,232,224,216};//posiciones para fase poner el char '^'
word n;
	...
	//ActividadMonitor(DESACTIVADO); //debug 
	 ...
	 VFDclrscr();// limpiar pantalla
	 VFDposicion(x[0],y[0]);delay1us(); 
	 switch(DDSpant->igxc3){//COMO INICIA EL DDS?
		 case 0xAA://first time displayed?
		           VFDserial_SendBlock(&r0[0],sizeof(r),&n);break;
		 default:           
				   VFDserial_SendBlock(&r[0],sizeof(r),&n);
				   DDSpant->igxc1=0;//no se usa 
				   DDSpant->igxc2=0;//indica si se ha hecho un cambio en sensibilidd o  phase u otra cosa y se tiene que repintar el DDS
				   DDSpant->igxc3=0xAA;//Itis init already, entro al menu por primera vez?
				   DDSpant->igxc4=0;//es el contrl del cuadro variable C=1|0
			       DDSpant->igxc5=0;//control de edicion de digitos de vars
				   cursorx=POSX_X;cursory=POSY0;
				   if(menu.contexto.padre==AJUSTE_PARAMETRICO_DE_PRODUCTO)
						 menu.contexto.Modificado=0;
				   break;
		 case 0xBB: //automatico PRoducto
//				  VFDposicion(0,POSY0);delay1us();  	  
//				  VFDserial_SendBlock(&m[0],sizeof(m),&n);
//				  VFDposicion(0,POSY2);delay1us();
//				  VFDserial_SendBlock(&m[0],sizeof(m),&n);
				  VFDposicion(0,POSY14);delay1us();
				  VFDserial_SendBlock(&k[0],sizeof(k),&n);
				  DDSpant->igxc1=0;//Activa el Procedimiento de Automatico
				  DDSpant->igxc2='U';//indica si se ha hecho un cambio en sensibilidd o  phase u otra cosa y se tiene que repintar el DDS
				  DDSpant->igxc3=0xCC;//Itis init already, entro al menu por primera vez?
				  DDSpant->igxc4=0;//controlaremos la configuracion automatica
				  DDSpant->igxc5=0;//Numero de Pasadas de Producto
				  dds.Bnderas.bit.AUTOMATIC=1;
				  cursorx=POSX_X;cursory=POSY0;//fin automatico
				  break;
//	    case 0xCC:VFDposicion(0,POSY0);delay1us();  	  
//				  VFDserial_SendBlock(&m[0],sizeof(m),&n);
//				  VFDposicion(0,POSY2);delay1us();
//				  VFDserial_SendBlock(&m[0],sizeof(m),&n);
				  VFDposicion(0,POSY14);delay1us();
...
//	 display_Linea_10_DDS(DDSaux);
	 ...
	 displayUINT_var(POSXDDSNUMS,6,&Deteccion.Altura,NONE);//despliega variable Altura deprecated	     displaySwitchGanancia_var(DDS,NONE);//el NONE es para decirle que solo lo despliegue sin el switching
	 ...
	 //dibujamos EJE de COORDENADAS, de las  lineas coordenadas graficas en el DDS
	 ...
	 display_Linea_Maestra_de_Phase();
 ...
 /* Asignacion de coordenadas de los cursores segun se la primera vez que se entra o se venga de un botonazo de borrado   */
	...
	//^--Funciono!! funcion bien no quitar
	 ...
	 switch(DDSpant->igxc2){
		case 0: if(cursory>POSY0){
					 VFDposicion(POSXDDSCURSORMENU,cursory);
					 VFDserial_SendChar('>');
					 cursorx=POSXDDSCURSORMENU;}
				 break;
		case 'S':if(DDSpant->igxc5>0)
    			      display_usint_cursor(DDSpant->igxc5,POSXDDSCURSORMENU,Deteccion.Sensibilidad);
    			 else{VFDposicion(POSXDDSCURSORMENU,POSY2);
				      VFDserial_SendChar('>');
				      cursory=POSY2;cursorx=POSXDDSCURSORMENU;}
				 break;
		case 'A':if(DDSpant->igxc5>0)
					  display_usint_cursor(DDSpant->igxc5,POSXDDSCURSORMENU,Deteccion.Altura);
				 else{VFDposicion(POSXDDSCURSORMENU,POSY6);
					  VFDserial_SendChar('>');
					  cursory=POSY2;cursorx=POSXDDSCURSORMENU;}
				 break;
		case 'P':if(DDSpant->igxc5>0){//no se esta editando digito individual
			                VFDposicion(POSXDDSCURSORMENU+16,POSY6);
			                VFDserial_SendBlock(&v[0],sizeof(v),&n);delay1us();
			                VFDposicion(h[(DDSpant->igxc5)-1],POSY6);
			                VFDserial_SendChar('^');}
		          else{VFDposicion(POSXDDSCURSORMENU,POSY4);
		               VFDserial_SendChar('>');
					   cursory=POSY4;cursorx=POSXDDSCURSORMENU;}
				 break;
		case 'B':  //cursor en borrar
				 ...
				 cursorx=POSXDDSCURSORMENU;
				 break;
		case 'U':switch(DDSpant->igxc4){//CONFIGURACION AUTOMATICA
			       case 0:DDSpant->igxc4=1;//iniciamos Controlador_Configuracion_Producto
			    	      break;
...
void DisplayParametrosdeDeteccion(void){
	 ...
	 word n;
		...
		displayUINT_var(POSXPARAMDETECT,POSY8,&Deteccion.Altura,NONE);//displayAltura_var(); falta de poner
		displayMarcarAltura(); 
		displayGanancia(); 
		...
		cursorx=POSX0;cursory=POSY4;
	    isEnable_Keypad(WAIT);//Desabilita el teclado uno milisegundos.
	    Reset_estructuras_Menu(&ParamDet);
	    ParamDet->igxc0=0;//Indica si se esta editando un numero
	    ParamDet->igxc1=0;//Indica cual numero POSY esta siendo editado
	    ParamDet->igxc2=0;//No asignado
	    ParamDet->igxc3=0;//No asignado
	    ParamDet->igxc4=0;//No asignado
	    ParamDet->igxc5=0;// Indica el digito que esta siendo editado
	    ParamDet->igxc6=0;//No asignado
...
unsigned char i,*s,j;	
	...
	switch(menu.contexto.Anterior){
	  ...
	  case NOMBRE_PRODUCTO:VFDserial_SendBlock(&t0[0],sizeof(t0),&n);delay1us();
	                       ...
	                       for(i=0,j=0;j<(NOMBRE_PRODUCTO_SIZE-1);i++,j++)//transferimos el nombre a la variable texto
	                             Text[i]=name[j];
	                       ...
	                       break;    
	  case SUPERVISOR:    VFDserial_SendBlock(&t1[0],sizeof(t1),&n);delay1us();
	  	  	  	  	  	  for(i=0;i<(NOMBRE_PRODUCTO_SIZE-1);i++)   
	  	  	                   Text[i]=0;//limpiamos la var.de texto
	  	  	  	  	      textProc->igxc0=MAYUSCULAS;//iniciamos desplegando mayuculas
	  	  	  	  	      textProc->igxc1=0;//indice del arreglo del Texto. 
...
/* esta funcion monitorea si se presiona una tecla para mandar o no el 
 * menu actual al portal inicio en base al timer.    */
void monitorActividad_IRQ(void){

	  timerPortalInicioFlag=TRUE;
	
}//fin monitor actividad IRQ-------------------------------------------
...
unsigned char  configModificacionController(unsigned char contextopadre,unsigned char contextomodificado1){
   unsigned char contexto; 
	switch(menu.contexto.Modificado){   //Nombre del Menu modificado??
	   ...
	   case AJUSTE_PARAMETRICO_DE_PRODUCTO:
		                  if(menu.contexto.Anterior==AJUSTES_AVANZADOS){
		                	  ...
		                	  break; }
		                  if(menu.contexto.Anterior==ESTAS_SEGURO){
		                	   menu.contexto.padre=MENU_INSIGHT;
		                	   menu.contexto.Anterior=TEXT_PROCESSOR;}
		                  if(menu.contexto.padre==MENU_INSIGHT)
		                	  if(menu.contexto.Anterior==TEXT_PROCESSOR){
	   ...
	   case NUEVO_PRODUCTO:
	   		   	   	   	   menu.contexto.Modificado=contextomodificado1;
	   		   	   	   	   contexto=ESTAS_SEGURO2;break;
	   case PANTALLA_DDS: if(contextopadre==AJUSTE_PARAMETRICO_DE_PRODUCTO){
		                        menu.contexto.padre=contextopadre;
	                            menu.contexto.Modificado=contextomodificado1;
	                            contexto=ESTAS_SEGURO2;
	                            return(contexto);}
		                  break;
	   default:// error de software
			   break;
	}//fin switch-------------------------------------------------
	menu.contexto.Modificado=0;//modificado=NOP;//se resetea la variable modificado.
return(contexto);
...
//	     static unsigned short int deteccionOFFSETcounter;//descuenta por tiempo una deteccion a la vez, a la variable que acomula las detecciones
	     ...
	     if((TM1_IRQ1&0x01)==0x01){
			...
			Control_Automatico_de_Barra_de_Deteccion_IRQ();
...
//			 LED12_SYS_NegVal(); 
	        LedController_IRQ();
	        keypad_Timer();
	        if(timer0.enable==RESET2){
	        	timer0.count=0;
	            timer0.count0=0;
	            timer0.enable=1;}
	        else{if(timer0.enable!=0)
	                  timer0.count++;}
	     }//bander de timer---------------------------------------------
...
/* controla automaticamente por timer la  autorizacion para 
 * despliegue e inicio de barra de deteccion y numeros por sistema operativo*/
void Control_Automatico_de_Barra_de_Deteccion_IRQ(void){//INT 500useg----------------------------
...
static unsigned char estado;
	 ...
	 switch(menu.contexto.Actual){
		 case AJUSTE_PARAMETRICO_DE_PRODUCTO:
			               switch(estado){
			            	   case 0:if(ncount++>200)//100 milisegundos espera
			            		            estado++;
			            	          break;
			            	   case 1:Deteccion.CuadroMadreReady=TRUE;
			            	          ncount=0;estado++;
			            	          break;
			            	   default:estado=0;ncount=0;break;}			                    
			              break;
		 default:estado=0;break;}
	
}//fin de Control_Automatico_de_Barra_de_Deteccion_IRQ------------------------------
 ...
 * 
 * */
void procSensxDigitoEN(unsigned char *digito,unsigned char posx,unsigned char initx,unsigned short int v){
unsigned char a[5],*p;	
      
	   switch(*digito){//  5 indica decena de miles y 1 unidad
	    				  case 1:*digito=0;p=&a[0];
	    					     cursorx=posx;
	    				         getCharsFromUINT_var(p,v);
...
/* Procesa la variable Sensibilidad, phase o altura  para restarle uno o sumarle uno
 * y lo despliega donde se le diga
 * POSY= 10xx xxxx indica la varible sensibilidad 
 * POSY= 11xx xxxx indica la varible phase
 * POSY= 01xx xxxx indica la varible Altura
 * POSY= xx1x xxxx indica suma
// * POSY= xx0x xxxx indica resta*/
//unsigned char Procesar_Operacion_y_Display_Var(unsigned char posx,unsigned char posy,unsigned short int *uint_var){
//unsigned char i,op;
//	 i=posy&0xC0; //var & 1100 0000, sacar los bits polisontes
//	 op=posy&0x20; //xx1x xxxx
//	 posy&=0x1FU;//limpiar la variable de bits polisontes
//     switch(i){
//    	 case 0x40:
//    	 case 0x80:if(op) procSensxDigitoRT(posx,posy,uint_var);
//    	           else  procSensxDigitoLF(posx,posy,uint_var);
//    	           break;
//    	 case 0xC0:procPhasexDigito(op);
//    	 default:__asm(Halt);}//error de software 
//    
//}//fin de procesadmiento de variables ----------------------------------
