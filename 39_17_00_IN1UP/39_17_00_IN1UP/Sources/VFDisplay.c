...
extern struct _GLOBAL_CONTEXTO g,*AjustVibr,*NewProd,*SelProd;
		...
		case AJUSTE_PARAMETRICO_DE_PRODUCTO:
							if(passControl[_AUTHOR_]==1){
							  ...
							  if(menu.contexto.Anterior==PORTAL_INICIO){
								   ...
								   if(menu.contexto.Anterior==PORTAL_INICIO)
									   if(menu.contexto.Actual==MENU_INSIGHT)
...
//		case AjustesAvanzadosAPP:
		...
		default:__asm(Halt);break;//debe ser un error, despues meter en controlador de errores                  
	}//fin switch destino------------------------------------------------         -
...
unsigned char n;	
	  ...
	  menu.contexto.Modificado=contxt;
...
/* Parametros,  var: variable valor usint a convertir en 5 chars
 * index: es la posciion del cursor que selecciona el digito individual */
...
unsigned char b[6]={" "},B,*bb;  
			p=&a[0];
			pushFIFOcOP_inicioCMD();//FIFO to display by S.O inicio cmd
			getCharsFromUINT_var(p,var);//get nums from a usint var, load vals to pointer
			convert2ascii(p,5);//convert numbers to asciis
			a[5]='>';
			B=*index;
			bb=&b[0];
			p=&a[0];
...
/*despliega el numero usint pero con su cursor en el digito
 *  que se esta editando
 *  param 1: digito a editar d, param2:posicion x donde se empieza a borrar
 *  param 2: pos x donde se escribe cursor y numero
 *  param3: variable a procesar v
 * */
void display_usint_cursor(unsigned char digit,unsigned char posx,unsigned short int var)
{unsigned char a[6];
 word n;
     getCharsFromUINT_var(&a[0],var);
     convertNum2Chars2(&a[0],5);
     VFDposicion(posx,cursory);
     VFDserial_SendChar(' ');
     VFDposicion((unsigned short int)(posx+16U),cursory);
     switch(digit){
    	 case 5:VFDserial_SendChar('>');
    	        VFDserial_SendBlock(&a[0],5,&n);break;
    	 case 4:VFDserial_SendChar(a[0]);
    	        VFDserial_SendChar('>');
    	        VFDserial_SendBlock(&a[1],4,&n);break;
    	 case 3:VFDserial_SendChar(a[0]);
    	        VFDserial_SendChar(a[1]);
    	        VFDserial_SendChar('>');
    	        VFDserial_SendBlock(&a[2],3,&n);break;
    	 case 2:VFDserial_SendBlock(&a[0],3,&n);
	            VFDserial_SendChar('>');
	            VFDserial_SendChar(a[3]);
	            VFDserial_SendChar(a[4]);break;
    	 case 1:VFDserial_SendBlock(&a[0],4,&n);
    	        VFDserial_SendChar('>');
    	        VFDserial_SendChar(a[4]);break;
    	 default:__asm(Halt);}//error de software debug
       //fin switch 	              
}//display_usint_con_cursor_edit_digito------------------------
...
unsigned char a[3];	
	getcharFromUChar(sensorBloqueado,&a[0]);
 ...
 * "esqx": es la posixcion x donde se grafica ">X y 
 * "posiciones" es el array de las posiciones 'autorizadas' 
...
unsigned char i,size,index;	
 ...
 //void (*pVFDposicion)(unsigned short x,unsigned short y);
        ...
        for(i=1;i<10;i++){   //buscar el tamaño del arrayv
        	if(*(posiciones+i)==0){
        		 size=i;
        	     break;}}
...
/* despliega las lineas de phase en angulo de 90 o  0*/
void displayLinea_Phase(unsigned char angulo,unsigned short int Cx){
const unsigned short int absisa_Max=96;

	if(angulo==90){if(Cx<absisa_Max)
		               VFDdrawLine(1,_CX+Cx,0,_CX+Cx,127);
	                   VFDdrawLine(1,_CX-Cx,0,_CX-Cx,127);}
	else{if(angulo==0){
		          if(Cx<64)
		        	  VFDdrawLine(1,0,Cx,192,Cx);
		              VFDdrawLine(1,0,Cx,192,Cx);}}
}//fin displayLinea_Phase-------------------------------------------


/*Cx es la distancia del origen(96,64) al punto
//donde se intersecta la recta con las absisas

*/
void display_Linea_Maestra_de_Phase(void){                                                   
float a,deg;
unsigned short int Cop;
float hypotenusa,m,Cx;
	  deg=convert_Phase_a_Real();//se  convirtio el numero fase en float
	  Cx=getCx(Deteccion.Sensibilidad,zoom.Zoom);
	  switch(get_angle_ctrl(deg)){
		  case 90: displayLinea_Phase(90,(unsigned short int)Cx);break;
		  case 0:  displayLinea_Phase(0,Cx); break;
		  default:a=convert_Deg_a_Rad(deg);//convertir de Grados a Radianes
         	      m=sin(a)/cos(a);  //tangente inversa del angulo
			      //displayLineAngulo(_CX,m); //linea central, para desarrollo,		 
				  Cop=(unsigned short int)Cx;     //distancia entre rectas, Cateto opuesto
				  hypotenusa=Cop/sin(a);
				  displayLineAngulo(_CX+hypotenusa,m,deg);
				  displayLineAngulo(_CX-hypotenusa,m,deg);
				  break;}
	   
}//fin display_Linea_Maestra_de_Phase---------------------------------------------------

/* calcula la distancia entre el origgen 96,64, del centro de la 
 *  pantalla y la recta de la sensibilidad en base al zoom actual 
 *  Las lineas paralelas Linea central y linea izquierda o derecha
 *    con las absisas forman un triangulo recto
 *   la hipotenusa es la distancia Cx entre las lineas medida por las 
 *   absisisas, */
unsigned short int getCx(unsigned short int Sens,unsigned char zoom){
const float pixel=0.3125;
const unsigned short int Max_Z1=32000;//maximo en zoom=1
const float Cx_z1_Max=86; //mm; cx es 27 cuando tenemos sens=32k en zoom=1
unsigned short int cx;
	switch(zoom){
		case 1:cx=(unsigned short int)((Cx_z1_Max*Sens)/(Max_Z1));
		       break;
...
/*dibuja una linea en base al centro y al angulo
 * m: pendiente */
void displayLineAngulo(float Cx,float m,float deg){
unsigned char s=0;//signo del grado
const unsigned char X=0,Y=1,XX=2,YY=3;
float cf[4];     
unsigned short int c[4],i;

	      if(get_coordenadas2(m,Cx,&cf[0],deg)){
	    	  for(i=0;i<4;i++)
	    	      c[i]=redondear(cf[i]);
	    	  if(verificar_coordenadas(&c[0])){
	    		  VFDdrawLine(1,c[0],c[1],c[2],c[3]);}}
}//displayLineAngulo-------------------------------------------------------------------------------
	    	   
//void displayLineAngulo(float Cx,float m){
//	      reset_usint(&c[0],4);
//          s=Formatear_Negativos(&a);//si pasa los 90 grados se procesa como negativo y se pasa entre 0 y 90
//		  a=convert_Deg_a_Rad(a);//convertir de Grados a Radiane  
//		  h=buscar_hypotenusa(Cx,Cy,a)
//		  x=h*cos(a);//coordenada x del punto P2 para graficar linea maestra
//		  y=h*sin(a);//debug coordenada Y del punto P2 para graficar linea de fase en angulo de fase con respecto a la absisa
//		  c[X]=redondear(x);//ux
//	      c[Y]=redondear(y);//uy
//	      verificar_coordenadas(&c[0]);
//	      get_coordenadas(s,&c[0],&centro[0]);//convertir catetos ADY y COP en x&y
//	      VFDdrawLine(1,centro[X],centro[Y],c[X],c[Y]);//CENTX+ux,CENTY-uy);  	 
//	      VFDdrawLine(1,centro[X],centro[Y],c[XX],c[YY]);
//	      
//fin VFDdrawLineAngulo------------------------------------------------------------------


/*evalua si el angulo es de 90  de 0 u otro mas como 90.1 u otro
 * regresa 90 si es 90 0 si es 0 y 0xff si es otra cosa*/
unsigned char get_angle_ctrl(float angulo){
	if(angulo==90)
		   return 90;
	else{ if(angulo==0)
		   return 0;
...
/* Dibuja la recta en DDS en base a un punto  y a la pendiente
 * Cx: centro x coordenada de la recta
 * a:angulo de la recta*/
//void displayLineaPunto(unsigned char pos,float angulo){
//	float m,x,y,Cx;
//const unsigned short int OX=192,OY=127;//ORILLAS DE PANTALLA
////unsigned short int c[4];//coordenadas a dibujar
//float xy[4],p1,p2,p3,p4;
////unsigned char n;
//const unsigned char DER_IZQ   =0x98;
//const unsigned char DER_BOTTOM=0x99;
//const unsigned char DER_UP    =0x9A;
//const unsigned char IZQ_BOTTOM=0x9B;
//const unsigned char IZQ_UP    =0x9C;
//const unsigned char UP_BOTTOM =0x9D;
//const unsigned char Y =0x10;
//const unsigned char XY=0x11;
//const unsigned char X =0x12;
//unsigned short int c[4];
//unsigned char marco,i;
//  if(pos==RIGHT){ //La linea parallela derecha
//    m=sin(angulo)/cos(angulo);//pendiente de la recta
//    Cx=CX+(Deteccion.Sensibilidad/2);x=OX;//despejar y & x
//    if(angulo==90){
//    	 if(Cx<192){
//    		   xy[1]=Cx;xy[2]=0;xy[3]=Cx;xy[4]=OY;}}
//    else{if(angulo==0){
//    	       xy[1]=0;xy[2]=Cx;xy[3]=192;xy[4]=Cx;}
//         else{    
//			p1=m*x-m*Cx;//y1,con x=Right=192 que y=?
//			x=0;                
//			p2=m*x-m*Cx;//y2, con x=Left=0 que y=? 
//			y=OY;
//			p3=(y/m)+Cx;//x1, con y=bottom=127 que x=?
//			y=0;
//			p4=(y/m)+Cx;//x2, con y=UP=0 que x=?
//			if((p1>0)&&(p1<OY)){//ver donde sale y entra la recta
//				if((p2>0)&&(p2<OY)){marco=DER_IZQ;}
//				if((p3>0)&&(p3<OX)){marco=DER_BOTTOM;}
//				if((p4>0)&&(p4<OX)){marco=DER_UP;}}
//			else{//marco izquierdo
//				if((p3>0)&&(p3<OX)){marco=IZQ_BOTTOM;}
//				if((p4>0)&&(p4<OX)){marco=IZQ_UP;}
//				else{marco=UP_BOTTOM;}}
////			get_coord2(&xy[0],m,(unsigned short int)Cx,marco); 
//			}}}
//   for(i=0;i<4;i++)
////      c[i]=redondear(xy[i]);
//   VFDdrawLine(1,c[0],c[1],c[2],c[3]);
//fin displayLineaPunto---------------------------------------------------------
...
unsigned char b[3]={0,0,0};
    ...
    if(mseg>99)
    	__asm(halt);//Debug error de software
    getcharFromUChar(segundo,&a[0]);
    getcharFromUChar(mseg,&b[0]);
    VFDposicion((unsigned short)(posx-8),posy);//
	switch(*digito){
		case 0xAA: //se activo la selecion del digito para modificalo
			      menu.contexto.Modificado=CONFIGURAR_ENTRADAS_DE_SISTEMA;// configModificado(CONFIGURAR_ENTRADAS_DE_SISTEMA);//Se mdifica un parametro
			      *digito=1;
			      VFDserial_SendChar(a[2]);
			      VFDserial_SendChar('.');
			      ...
			      break;
		case 0:   menu.contexto.Modificado=CONFIGURAR_ENTRADAS_DE_SISTEMA;//Se mdifica un parametr
				  VFDserial_SendChar(' ');
				  VFDserial_SendChar(a[2]);
				  VFDserial_SendChar('.');
				  VFDserial_SendChar(b[1]);
				  VFDserial_SendChar(b[2]);
				  break;
		case 1:   menu.contexto.Modificado=CONFIGURAR_ENTRADAS_DE_SISTEMA;//Se mdifica un parametr
				  VFDserial_SendChar('>');
		          VFDserial_SendChar(a[2]);
				  VFDserial_SendChar('.');
				  VFDserial_SendChar(b[1]);
				  VFDserial_SendChar(b[2]);
	              break;
		case 2:   menu.contexto.Modificado=CONFIGURAR_ENTRADAS_DE_SISTEMA;//Se mdifica un parametro
				  VFDserial_SendChar(a[2]);
				  VFDserial_SendChar('.');
				  VFDserial_SendChar('>');
				  VFDserial_SendChar(b[1]);
				  VFDserial_SendChar(b[2]);
				  break;
		case 3:   menu.contexto.Modificado=CONFIGURAR_ENTRADAS_DE_SISTEMA;//Se mdifica un parametro
				  VFDserial_SendChar(a[2]);
				  VFDserial_SendChar('.');
				  VFDserial_SendChar(b[1]);
				  VFDserial_SendChar('>'); 
...
void DisplayCharTextProc(unsigned char pos,unsigned char dato){
	  VFDposicion((unsigned short)((pos)*8),POSY2);
...
l9:  if(numero[i]==0){
	       ...
	       goto l9;}
    	        ...
    	        FIFO_displayBoxChar_push(pos,POSY12);
lA:    	        FIFO_displayBoxChar_push(numero[i]+0x30,0xFE);
    	        if(i==(SIZE1-1))
    	        	return;
    	        i++;
    	        goto lA;}
...
//uword2 v;
       ...
       VFDserial_SendChar_DDS('.');
       VFDserial_SendChar_DDS(phasefrac+0x30);	   
...
unsigned char *p,phase,phasefrac; //para los valores ascii del numero entero
        p=&a[0];phase=Deteccion.phase;phasefrac=Deteccion.phasefrac;
	    getUChar2Chars(p,phase,YES);//obtiene los tres char de un numero char unsigned
        if(phasefrac>9)
	          phasefrac=0;
    	//debug    if(posx==0)
...
/* evalua si hay mas de un bit encendido si lo hay primero procesa
 *  el LSB  y luego hacia el MSB por prioridad */
//unsigned char procesarPrioridad(unsigned char n){
//const unsigned char a[]={0x01,0x02,0x04,0x08,0x10,0x20,0x40,0x80};
//unsigned char i;        
//	  for(i=0;i<8;i++){
//		    if((n&a[i])==a[i])
//		    	return a[i];}
//return 0;		  
//}//procesarPrioridad FIN -------------------------------------------------------

//Desplega datos usando el sistema operativo cuando hay una barra 
void displayUINT_var_con_BarraDet(unsigned char posx,unsigned char posy,unsigned short int *usint){
...
//Mete ala FIFO de despliegue la varible phase para desplegarla en x & y
void display_Phase_var_con_BarraDet(unsigned char posx,unsigned char posy){
unsigned char a[5],*p,n=0;
		p=&a[0];
		getASCII_from_Fase(p,Deteccion.phase,Deteccion.phasefrac);
		VFDposicion_DDS(posx,posy);		
		VFDserial_SendChar_DDS(*(p+0)); //decmil+0x30);
		VFDserial_SendChar_DDS(*(p+1));//mil+0x30);
		VFDserial_SendChar_DDS(*(p+2));//cent+0x30);
		VFDserial_SendChar_DDS(*(p+3));//dec+0x30);
		VFDserial_SendChar_DDS(*(p+4));//unidad+0x30);	  
	   // if(n>0)
	     //  __asm(HALT);//debug alto, error de ingenieria de software
}//fin BarraDet_displayUINT_var-------------------------------------------------------------
...
unsigned long int i;
	...
	#endif
				if(prod.cntrl.iProduct>5){
					LimitesMenuPROD(prod.cntrl.iProduct-4,ESCRIBIR);  //ver que prducto esta seleccionado para asignar lo limites superiores
					cursorx=POSXSELPRODCURSORX;cursory=POSY10;}
				else{ LimitesMenuPROD(1,ESCRIBIR);
					  cursorx=POSXSELPRODCURSORX;cursory=prod.cntrl.iProduct+prod.cntrl.iProduct;}
			    fillScreenNames();
...
void displayGanancia(void){displayfrecuencySelect_var();}
void displayMarcarAltura(void){
unsigned char a[]="Si";
unsigned char b[]="No";
const unsigned char posx=136,posy=POSY10;
word n;
     VFDposicion(posx,posy);
	 if(dds.Bnderas.bit.MarcarAltura)
		 VFDserial_SendBlock(&a[0],sizeof(a),&n);
	 else VFDserial_SendBlock(&b[0],sizeof(b),&n);
}//fin displayMarcarAltura------------------------------------
	 ...
	 switch(menu.contexto.padre){
	    ...
	    case NUEVO_PRODUCTO:
	    	               if(menu.contexto.Anterior==AJUSTE_DE_PRODUCTO_MODE){
	    	            	   p=getNameiProduct(arg0);//abajo, save product actual							   
							   VaciarArray_A_to_B(p,&name[0],NOMBRE_PRODUCTO_SIZE);}
	    	               break;
	    default:return;break;} 
	 if(name[0]==NAME_INIT)
		   k=1;
	 else k=0;
	 for(i=k;i<sizeof(name);i++){//ver tamaño del nombre
	        if(name[i]==10)
	              break;
	        else{if(name[i]==0)
	                 break;}}
