...
//unsigned short int x1[600],y1[600];
const unsigned char SIZE_APP=20;
unsigned long int app[SIZE_APP];
extern struct _TIMER_ timer1;
...
//	             SignalProcessor_IRQ();//Procesamiento de datos digitales del ADC
	    timeApp(0,&app[0]);
	    DDSsignalProcessor_Operativo();timeApp(1,&app[0]);// se tiene que inizializar el dsp //despliega los puntitos en pantalla DDS
	    DDS_repaint_Zoom();timeApp(2,&app[0]);//aqui,	    
	    Display_DDS_TRANSMISSOR_CONTROLLER();timeApp(3,&app[0]);//transmite los datos al menu DDS
	    IOUP_BOARD_SERIAL_CONTROLLER();timeApp(4,&app[0]);//CONTROLADOR PRINCIPAL DE transmision a la tarjeta io
	    Signal_Processor_Controller();timeApp(5,&app[0]);//Procesador Central de las Señales analogas digitalizadas
	    MonitorCentralEntradas();timeApp(6,&app[0]);//monitorea las ntradas y decide 
	    displayPruebasEntradas();timeApp(7,&app[0]);  
	    //ControladorOperativoPrincipal()
	    displayOperativoBarraDet_Numeros();timeApp(8,&app[0]);//deprecated Display&Cntrl de datos del Menu de Configuracion de angulo de Vibracion 
	    Display_Y_Barra_Deteccion_Controlador_Operativo();timeApp(9,&app[0]);//despliegue de barra de deteccion y caracteres
	    Control_Operativo_de_Escape_de_Menus();timeApp(10,&app[0]);//Cambio de Menu  
	    menuTemporizadorOperativo();timeApp(11,&app[0]);//monitor de menus,
	    ErrorMonitor();timeApp(12,&app[0]);//monitor de errores
...
//	    Procesador_Operativo_de_Graphicos();EN CONSTRUCCION
	    ControladorOperativoTemporizado_IRQ();timeApp(13,&app[0]);//controlador de tiempos del sistema operativo
	    displayTimer_IRQ();timeApp(14,&app[0]);
	    IN3UP_test();timeApp(15,&app[0]);
	    DebugSimulationKeyboard();timeApp(16,&app[0]);//teclado virtual remoto
	    Monitor_Maestro_de_Voltajes();timeApp(17,&app[0]);//monitorea los voltajes de la analoga.
	    
	    Controlador_Principal_Central_Communicacion_IIC();timeApp(18,&app[0]);
	    //Controlador_de_Teclado_Keypad();

	    Controlador_Configuracion_Producto();timeApp(19,&app[0]);
