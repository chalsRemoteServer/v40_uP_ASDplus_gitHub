...
//				  contextoModificado=0;}
  ...
  if(menu.bytes.semMenuPendiente==TRUE){//hay un menu pendiente para salir de alli
    switch(menu.contexto.Anterior){//de donde venimos
    	case PORTAL_INICIO: menu.contexto.Anterior=PORTAL_INICIO;goto ctlop1;
    	case AJUSTE_PARAMETRICO_DE_PRODUCTO:switch(menu.contexto.Actual){
    											  case   PANTALLA_DDS: menu.contexto.padre=AJUSTE_PARAMETRICO_DE_PRODUCTO;
    											                       break;
    		                                      default:break;  }
    	case AJUSTE_DE_VIBRACION:
    	case PARAMETROS_DE_DETECCION:
ctlop1:    		              if(Deteccion.BarraDeteccionStatus==BUSY)
    		            	  return;
    	...
    	case PANTALLA_DDS: if(dds.Bnderas.bit.Apagando==TRUE)
    					   ...
    					   menu.debug43=0x0B;
						   break;
    	case MENU_INSIGHT:if(menu.contexto.Actual==PANTALLA_DDS)
    		                   menu.contexto.padre=PORTAL_INICIO;
    	                  DisplayMenu();
    				      menu.bytes.semMenuPendiente=FALSE;//limpiamos semaforos
    				      break;
    	case TEXT_PROCESSOR:__asm(nop);
    	case ESTAS_SEGURO:
        case ESTAS_SEGURO2:
...
if(semaforoDDS(READ)==VERDE){
 ...
 if(menu.contexto.Actual==PANTALLA_DDS){//debe entrar aqui despeus de un tiempo depues que se inizialize las fifos.
	 if(DDSpant->igxc2=='B') //dds.Bnderas.bit.DDS1_BORRAR==TRUE)
