...
//unsigned short int  Deteccion.Altura; //deprecated,    variable de parametros de deteccion------para el DDS
...
unsigned char a1[]=" Salchichas asd";
...
//    indiceProducto=1;//producto Set TestUp, first run.
    menu.contexto.Modificado=0; //modificado=NOP;//variable que indica que no se ha modfificado la configuracion en el menu
   ...
   //ESPACIO A+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
         prod.cntrl.iProduct=1; //indiceProducto=1;//producto Set TestUp, first run.
   ...
   //ESPACIO B++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
         ...
         p=&buffer[0];p2=p;
         *p=prod.cntrl.iProduct =1;//indiceProducto=1;                       //00H  Test SetUp
...
//return TRUE: si la password ingresada no esta duplicada en algun otro espacio de passwords
unsigned char validatePasswords(unsigned char *Level,unsigned char *T){//validamos la password
unsigned char *p;
unsigned char i;	

    if((*T=='5')&&(*(T+1)==0)){//la clave es 5=servicio
    	*Level=5;
    	return FALSE;} 
    for(i=1;i<=4;i++){// cuatro niveles de password
       p=getPasswords(i); 
       if(compareString(T,p)){//si son iguales minimo una se rompe el for y regresara false todo
	        *Level=i;//el nivel de la password que coincidio,
    	    return FALSE;}} //regresa false, porque hay una igual, eso es malo poreso regresa false
return TRUE; //no hubo ninguna password igual------------    
 ...
 switch(op){
   case SAVE://y cambiamos al otro producto iProd
	...
	//		   p=readEEPROMblock(ESPACIO_C_ADD,ESPACIO_C_SIZE);//leemos espacio Cache
			   writeEEPROMblock(PROD_VAR0_ADD+(PROD_VAR_SIZE*(prod.cntrl.iProduct-1)),PROD_VAR_SIZE,p);//escribimos espacio de producto con sus valores de cache
...
void SacarVar_Espacio_B_to_systemVarRAM(unsigned char *p){
	                      ...
	                      prod.cntrl.iProduct=*p;                  //00H  Test SetUp
...
//Dword word32;    //disponemos de 128kbytes
       ...
       prod.cntrl.iProduct0=prod.cntrl.iProduct;//Se guarda por si se arrepienten de crearlo y no quieren guardar cambios
      ...
      if(prod.cntrl.iProduct==1)//esta actualmete el TestSeptup puesto
      ...
      prod.cntrl.iProduct=iProd;//actualizamose el producto nuevo que ahora es el seleccionado
...
//      AsignVarProdAndGetPointer(p,p2);//escribimos el nombre p2 y obtenemos los valores estndard *p de las var RAM de prod
      ...
      writeEEPROM_Pn(prod.cntrl.iProduct,p);//escribimos producto en espacion Pn 
//      VaciarArray_A_to_B(p2,&Name[0]);  
...
/*Sacamos el nomnre del producto selecciona por parametro iProduct*/
unsigned char *getNameiProduct(unsigned char iProd){
	   return(readEEPROMblock(PROD_VAR0_ADD+(PROD_VAR_SIZE*(iProd-1)),PROD_VAR_SIZE));
}//fin getNameiProduct-------------------------------------------------
...
//Asisgnacion de VAriables de Producto POrPRIMERA VEZ A ESPACIO CACHE
                        ...
                        *(p+24) =Deteccion.phase=1;     
                        *(p+25) =Deteccion.phasefrac=5;         //1DH
                        *(p+26) =prod.cntrl.frecuenciaSeleccion=MEDIA;                           //1EH
                        *(p+27) =MarcarAltura='0';                      //1FH
                        *(p+28) =GananciaDDS='0'; 
                        *(p+29) =zoom.Zoom=1;               //29H    
                        *(p+30) =cuadritoDDS='0';             //2AH  
                         word32.wordx=Deteccion.Sensibilidad=25;// OOM_MAX_Y_99;
                         *(p+31)=word32.byte[0];
                         *(p+32)=word32.byte[1];
                         word32.wordx=Deteccion.Altura=32000;
                         *(p+33)=word32.byte[0];
                         *(p+34)=word32.byte[1];
...
//Asisgnacion de VAriables de Producto POrPRIMERA VEZ A ESPACIO CACHE
                        ...
                        *(p+30) =cuadritoDDS='0';             //2AH  
                         ...
                         *(p+31)=word32.byte[0];
                         *(p+32)=word32.byte[1];
                         word32.wordx=Deteccion.Altura=31234;
...
uword word32;  
       ...
       Deteccion.phase              =*(p+24);
  Deteccion.phasefrac      =*(p+25);         //1DH
...
uword word32;   
              ...
              //  *(p+23) //=BorrarContadores; libre se paso a varible de sistema
                *(p+24) =Deteccion.phase;     
                *(p+25) =Deteccion.phasefrac;         //1DH
...
//unsigned char dat[10]; // ESCRITURA Y LECTRURA ES POR PANTALLA  <<---NOTA!!!!!!!!!!!!
    ...
    switch(contexto1){//ES POR PANTALLA NO POR VARIABLE INDIVIDUAL
       ...
       case AJUSTE_DE_PRODUCTO:
    	                    if(operacion==SAVE){
    	                         if(Deteccion.phasefrac!=arg0)
    	                        	 writeEEPROMbyte2(ESPACIO_C_ADD+25,Deteccion.phasefrac);
    	                         if(Deteccion.phase!=arg1)
    	                        	 writeEEPROMbyte2(ESPACIO_C_ADD+24,Deteccion.phase);
    	                         if(Deteccion.Sensibilidad!=arg3){
    	                             word32.wordx=Deteccion.Sensibilidad;
                                     p=&word32.byte[0];
                                     writeEEPROMblock(ESPACIO_C_ADD+31,2,p);}
    	                         if(Deteccion.Altura!=arg7){
    	                        	 word32.wordx=Deteccion.Altura;
    	                        	 p=&word32.byte[0];
       ...
       case NUEVO_PRODUCTO:if(operacion==SAVE){
    	   	   	   	   	   	    ...
    	   	   	   	   	   	    if(prod.cntrl.iProduct==0)
    	   	   	   	   	   	    	__asm(Halt);//debug error de diseño de software
    	   	   	   	   	   	    if(prod.cntrl.iProduct>1){
...
//                          word32.wordx=frecuency;
       ...
       case RELOJ_DE_SISTEMA: saveTimeNVRAM(); //año,mes,dia,fecha,hora,minutos,segundos;break; 
                            break;
       case PARAMETROS_DE_DETECCION://1<Deteccion.Altura<32000, MarcarAtura=No|Si
                      if(operacion==RESTAURAR){
                         ...
                         word32.byte[2]=*(p+2);
                         word32.byte[3]=*(p+3);
                         Deteccion.Altura=word32.wordx;//deprecated
                         MarcarAltura=*(p+4);
                         I2CliberarModulo();
                         p=readEEPROMblock(add,FRECUENCIA_SELECT_SIZE);
                         frecuenciaSeleccion=*p;
                         ...
                         Deteccion.phase=*(p+5);
                         Deteccion.phasefrac=*(p+6);
                         I2CliberarModulo(); 
                         if(Deteccion.Altura>32000)
                                Deteccion.Altura=32000;
...
unsigned char *p;   
    ...
    writeEEPROMblock(PROD_VAR0_ADD+(PROD_VAR_SIZE*(prod.cntrl.iProduct-1)),PROD_VAR_SIZE,p);
...
unsigned char *DebugVirtualNVRAM(unsigned char op,unsigned short int add,unsigned char *w,
		unsigned short int nbytes){
static const unsigned short int SIZE_EEPROM=56;
static unsigned char NVRAM[SIZE_EEPROM];	
unsigned short int i;	      
