...
unsigned char  buffer3[SIZE_BUFFER2];//fifo de i2c op, operation
unsigned char  buffer4[SIZE_BUFFER2];//FIFO DE I2C addl, I2C,
unsigned char  buffer5[SIZE_BUFFER2];//fifo  i2c,  addh, I2C,
void (*fbuffer[SIZE_BUFFER2])(void);//buffer pointers to function
...
void init_queues(void){
	//unsigned char c;
    	  ...
    	  init_FIFO_General_1byte(&serial.rx,&buffer2[0],&buffer2[SIZE_BUFFER1-1],SIZE_BUFFER1);
     	  ...
     	  init_FIFO_General_1byte(&i2c.op,  &buffer3[0],&buffer3[SIZE_BUFFER2-1],SIZE_BUFFER2);
     	  init_FIFO_General_1byte(&i2c.addl,&buffer4[0],&buffer4[SIZE_BUFFER2-1],SIZE_BUFFER2);
     	  init_FIFO_General_1byte(&i2c.addh,&buffer5[0],&buffer5[SIZE_BUFFER2-1],SIZE_BUFFER2);
     	  init_FIFO_General_vfv(&i2c.f,&fbuffer[SIZE_BUFFER2-1],SIZE_BUFFER2);//void f(void)
...
void init_FIFO_General_1byte(struct _FIFO_1byte_ *s,unsigned char *h,unsigned char *t,unsigned char size){//FIFO_SER_KEYPAD[SIZE_SER_KEYPAD];
	s->head=h;
	s->tail=t;
	s->pop=s->head;
	s->push=s->head;
...
/*parametro    FIFO a array de funciones void f(void)
 * 1: La fifo a inizializar
 * 2: pointer to  array of pointers to functions
 * 4: size of fifo 
 * version 310322-1637 add reset as general */
void init_FIFO_General_vfv(struct _FIFO_void_f_void_ *s,void (**fpp)(void),unsigned char size){//FIFO_SER_KEYPAD[SIZE_SER_KEYPAD];
	s->fpp=fpp;
	s->head=0;
	s->tail=size-1; //(fpp+(size-1));
	s->pop=s->head;
	s->push=s->head;
	s->ncount=0;
	s->popf=FIFO_general_vfv_pop;
	s->appendByte=FIFO_general_vfv_push;
	s->size=size;
	s->resetFIFO=reset_FIFO_general_vfv;
}//fin init_FIFO_RX_serial_Keypad-----------------------------------
...
void reset_FIFO_general_UChar(struct _FIFO_1byte_ *s){
	   s->push=s->head;
	   s->ncount=0;
	   for(s->pop=s->head;s->pop<=s->tail;s->pop++)
		   *(s->pop)=0;
	   s->pop=s->head;
}//fin reset_FIFO_serial_TX---fin se resetea toda la fifo

/*vfv = void funct(void)*/
void reset_FIFO_general_vfv(struct _FIFO_void_f_void_ *s){
	   s->push=s->head;
	   s->ncount=0;
	   for(s->pop=s->head;s->pop<=s->tail;s->pop++)
		   *(s->fpp+s->pop)=0;
...
//return FALSE if is empty
/* version  void func(void) fifo de array de pointer to funcintions */
unsigned char FIFO_general_vfv_pop(void (*f)(void),struct _FIFO_void_f_void_ *s){	
	if(s->ncount==0)
		return FALSE;
	f=*(s->fpp+s->pop);//solo hay un dato en la FIFO//i2c.
	*(s->fpp+s->pop)=0;//vaciamos nodo
	switch(get_case_FIFO_general_vfv(s)){
		case 33:
		case 1:s->ncount=0;
		       break;
		case 6:       
		...
		case 7:
		case 8:s->ncount--;
		       s->pop=(s->pop)+1;//un lugar hacia abajo
		       break;	
		default: __asm(Halt); break;}//error de programacion}//fin switch
return TRUE;
}//FIFO_general_1byte_push------------------------------------------
...
unsigned char FIFO_general_vfv_push(void (*f)(void),struct _FIFO_void_f_void_ *s){
	 if(s->ncount<s->size)
		 goto  lqwe379;
	  else return FALSE;//FIFO llena
lqwe379:
	switch(get_case_FIFO_general_vfv(s)){
		case 5:
		case 33:*(s->fpp+s->push)=f;//s->push=f;
	            s->ncount++;
	            s->push=s->head;
			    break;
		case 1:*(s->fpp+s->push)=f;
			   s->ncount++;
			   if(s->push==s->tail)
				   s->push=s->head;
			   else s->fpp++; 
		       break;
		case 7:       
		case 2:*(s->fpp+s->push)=f;
		       s->ncount++;
		       s->push++;
		       break;
		case 3:*(s->fpp+s->push)=f;//nomas cabe un dato, este dato y ya
	           s->ncount++;
	           break;
		case 8:       
		case 6:       
		case 4:*(s->fpp+s->push)=f;//nomas cabe un dato, este dato y ya
               s->ncount++;
               if(((s->push)+1)!=s->pop)//el que sigue es pop, 'tons'ta llena
            	    s->push++;
               break;	
...
/* version para fifo voif func void*/
unsigned char get_case_FIFO_general_vfv(struct _FIFO_void_f_void_ *s){
	 if(s->pop==s->push)
		    return 1;//
