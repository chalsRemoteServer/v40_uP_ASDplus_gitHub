...
const unsigned char BUSY2=0x23;
const unsigned short int BLINCK=1000;//blinck en estado monitor
const unsigned short int DETECT=300;
    switch(n){
    	case ACTIVADO: estado=ACTIVADO;break;
	    case BUSY:	   if(estado!=BUSY2)
							 estado=BUSY;
					   break;
	    case ON:       LED3_Process_PutVal(FALSE);//encender led 3
	                   return;
    	case OFF:      estado=OFF;break;
    	default:break;}
    switch(estado){
    	case ACTIVADO:if(cont++>DETECT){
    		             cont=0;
    		             LED3_Process_NegVal();}
    	              estado=n;
    	              break;
    	case BUSY:   if(cont++>BLINCK){
                         cont=0;
                         LED3_Process_PutVal(TRUE);
					     estado=BUSY2;}
    				 else estado=BUSY;
					 break;          
    	case BUSY2:  if(cont++>BLINCK){
						  cont=0;
						  LED3_Process_PutVal(FALSE);
						  estado=BUSY;}
					 else estado=BUSY2;
					 break;   
    	case OFF:    LED3_Process_PutVal(TRUE);//apagar          
    	             cont=0;
    	default:	 break;}
 ...
 *             timestamp,[tipo:Warnning|Error|info],param1:Conteo,
 *             param2:nivel deteccion);<--Se van a guardar en la NVRAM
 * */
