 ...
 * */
void Display_de_Graficos_Operativo(void){
static unsigned char estado,estadowait;
unsigned char x=0,y=0,p=0,*box1,*box0;
unsigned short int x1,y1,x2,y2;
unsigned char pen,mode,ibox0;	
coordn16 coordenadas;
const unsigned short int DELAY_TIME=100;//ENTRE CAda chars TRANSMISION AL vfd

	  if(menu.bytes.semMenuPendiente==TRUE){
	      if(Deteccion.BarraDeteccionStatus==BUSY)
                goto maquinaEstados;//termina de graficar, correle.
	      else return;} //vamos a cambiar  de menu y no podemos graficar nada
	  //no hay menu pendiente, estado normal de graficacion
maquinaEstados:
      ...
      switch(estado){
    	  case 0: if(vfd.f.pop(&x,&y,&p)){ //No esta vacio
    		      ...
    		      break;
    	  case 1: switch(p&0xF0){
					  case 	_BOX_:*box1=vfd.v.dat[13];estado++;break;
    	  ...
    	  case 2:if(*box0>MAX_BOXES)
				 ...
				 estado=33;//emitir los datos;
				 break;//fin case 2------------------------------------
    	  case 3:vfd.v.dat[0]=vfd.v.dat[13]; //x=vfd.v.dat[13];y=vfd.v.dat[12];p=vfd.v.dat[11];			 
			     vfd.v.nbytes=1;//bytes a emitir
                 estado=33;
                 break;
    	  case 4:vfd.v.dat[0]=0x1F;
				 vfd.v.dat[1]=0x24;
...
}// Display_de_Graficos_Operativo---------------------------------------(void){
