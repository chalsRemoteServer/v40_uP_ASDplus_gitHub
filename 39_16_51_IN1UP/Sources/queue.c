...
void init_queues(void){
	//unsigned char c;
    	  ...
    	  init_FIFO_General_1byte(&serial.rx,&buffer2[0],&buffer2[SIZE_BUFFER1-1],SIZE_BUFFER1);
     	  ...
     	  vfd.f.append= vfd_FIFO_push;
     	  vfd.f.pop=vfd_FIFO_pop;
...
static unsigned char i,c[3];
  switch(menu.contexto.Actual){
	  case PARAMETROS_DE_DETECCION:
	  case PORTAL_INICIO:
				if(cont++>1000){
//					pushFIFOcOP_inicioCMD();
					vfd.f.append(64,POSY6,_POS_);// pushFIFOcOP_VFDposicion(64,POSY6);
					if(++i>250)
						i=0;
					getASCII_from_UChar(i,&c[0]);
					vfd.f.append(c[0],0,_CHAR_);//pushFIFOcOP_VFDserial_SendChar(c[0]);
					vfd.f.append(c[1],0,_CHAR_);//pushFIFOcOP_VFDserial_SendChar(c[1]);
					vfd.f.append(c[2],0,_CHAR_);//pushFIFOcOP_VFDserial_SendChar(c[2]);
					//pushFIFOcOP_finCMD();
					cont=0;}
		  break;
	  default:break;}	  
...
unsigned char vfd_FIFO_push(unsigned char x,unsigned char y,unsigned char p){
    ...
    switch(p&0xF0){//1100 0000 los dos MSB indican que parametro es	
    	case _BOX_:if(vfd.box.boxs[x]!=p)//la caja ya se ejecuto este comando ?
    					vfd.box.boxs[x]=p;
    	            else break;
    	case _CHAR_:
    	case _PUNTO_:
    	case _RAYA_: 
    	case _POS_ : 
    		         vfd.x.appendByte(x,&vfd.x);
    		         vfd.y.appendByte(y,&vfd.y);
    		         if(!vfd.p.appendByte(p,&vfd.p)){
       	                 __asm(nop);
    		        	 __asm(Halt);
    		             __asm(nop);}
                     break; 
