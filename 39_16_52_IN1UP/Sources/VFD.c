...
void VFDserial_SendChar_DDS(unsigned char c){
	vfd.f.append(c,0,_CHAR_);// FIFO_Display_DDS_Char_push(c,0xFE);//0xFE means that is just a char display          
...
void VFDposicion_DDS(unsigned short int x,unsigned short int y){ //MANDA DEL COMANDO DE POSICION AL vfd
	vfd.f.append(x,y,_POS_);//FIFO_Display_DDS_Char_push((unsigned char)x,(unsigned char)y);          
}// fin posicionVFD-------------------------------------------------------------
...
void VFDcommand_Bold_DDS(unsigned char bold){
	vfd.f.append(0,0,_BOLD_);// FIFO_Display_DDS_Char_push(bold,0xFD);//FDh means that we'll send a command of kind bold
}//fin VFDcomando DDS-------------------------------------
