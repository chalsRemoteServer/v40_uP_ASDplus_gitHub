...
void init_queues(void){
	//unsigned char c;
    	  ...
    	  init_FIFO_General_1byte(&serial.rx,&buffer2[0],&buffer2[SIZE_BUFFER1-1],SIZE_BUFFER1);
     	  ...
     	  vfd.f.resetFIFOS=vfd_FIFOs_RESET;
...
//					pushFIFOcOP_inicioCMD();
					vfd.f.append(152,POSY12,_POS_);// pushFIFOcOP_VFDposicion(64,POSY6);
					if(++i>250)
						i=0;
...
const unsigned char BYTES_BOX=150; //numero de ciclos, mas que bytes por comando de una box cdraw 
unsigned char n=0;	
    switch(p){//1100 0000 los dos MSB indican que pqqrametro es
    	case _BOX_:if(x==0)
    		             return;
    	           if(vfd.box.timer==0){
