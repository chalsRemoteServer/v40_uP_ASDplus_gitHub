...
void Planificador_del_Sistema_Operativo(void);
unsigned char find_contexto_Siguiente(unsigned char *Contexto,unsigned char *inst);
unsigned char busqueda_de_variables_Modificadas(unsigned char *inst);
unsigned char newpid(void);
byte create(byte (*process)(unsigned char *m),char *name,unsigned char size,unsigned char prior,unsigned char  PadrePID);
void kill(byte pid);
void suspend(byte pid);
void resume(byte pid);
void insert_Recurso_en_Proceso(byte recurso,byte pid);
void removeRecurso(byte pid);
//byte buscarRecurso(byte recurso,byte pid,byte *index);
void cleanRecursos(byte pid);
//byte Booting_System(void);
byte startup_System(byte *inst);
byte Loading_Operative_System(byte *inst);
byte System_Configuration(byte *inst);
byte Load_System_Utilities(byte *inst);
byte BIOS_Power_On_Self_Test(byte *inst);
byte Load_System_Utilities(byte *inst);
void add_PID(byte pid);
byte Booting_System(byte *inst);
byte next_PID(void);
void init_SO(void);
byte System_CaLL_SeekFile(const char *name,byte pid,byte *mem);
byte obtener_timer_libre(byte *t);
void sleep(word time);
byte init_Perifericos(byte *inst);
void free(byte pid);
byte run(byte pid);
