...
//COMANDO PARA EL DRIVER DE LA MEMORIA EEPROM
#define CMD_WRITE_E2 0x44
#define CMD_READ_E2  0xDF
...
//void init_Memoria_Menu(struct Displ1 *v,unsigned char size);
unsigned char validatePassword_v2(unsigned char *p);
//unsigned char *Buscar_Asignar_Proc_Memo_Libre(byte size);
byte get_size_del_Bloque(byte *pBloque);
//void limpiar_memoria(byte *p,byte size);
byte* get_Addr_de_la_Tabla_de_asignacion(byte *bloque);
//byte* malloc_memory(byte  size);
void free_memory(byte *p);
//void update_pid_Tabla_de_asignacion(byte *addressTabla,byte sizeTabla);
byte* get_Addr_block_Memo(byte *pProc);
//byte *get_Addr_Tabla_Asignacion(byte *pBloque,byte *sizeBlock,byte *sizeTabla);
//unsigned char buscar_Asignar_espacio_pid(unsigned char *p,unsigned char pid,unsigned char sizeTabla);
byte Controlador_Driver_de_EEPROM(byte *inst);
void iif(void);
//byte  *buscar_bloque_de_memoria_enBanco_de_memoria(byte blocksize,byte offset);
byte* malloc_memory(byte  *size,byte pid);
byte get_index_of_pid_in_Asignation_table(byte *pidAddrBlock,byte *addHeadBlock,byte sizeBlock);
//void update_pid_Tabla_de_asignacion(byte *addressTabla,byte sizeTabla);
//void Init_Memoria_Procesos(void);
//struct _Nodo *BTNF(struct _Nodo_ *n,struct _Nodo_ **p);
