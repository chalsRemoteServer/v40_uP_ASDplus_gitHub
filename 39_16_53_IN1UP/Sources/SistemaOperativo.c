    ...
    switch(menu.contexto.Anterior){//de donde venimos
    	case SUPERVISOR:
    		   switch(menu.contexto.Actual){
    			   case TEXT_PROCESSOR:DisplayMenu(); 
		...
		case AJUSTE_TIEMPO_RECHAZO:
			               //ventana que pide la clave para entrar a un nuevo menu	
			               DisplayMenu();//Despliegua  ESTAS-SEGURO2?
			               menu.bytes.semMenuPendiente=FALSE;//limpiamos semaforos
			               menu.debug43=0x0C;
 ...
 * */
void Controlador_Driver_de_Display_Graficos_VFD_Operativo(void){
static unsigned char estado,estadowait;
unsigned char *x,*y,*p,*box1,*box0;
unsigned short int x1,y1,x2,y2;
unsigned char pen,mode,ibox0;	
coordn16 coordenadas;
static short int cleanerBox;
																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																												

	  if(menu.bytes.semMenuPendiente==TRUE){
		  if(Deteccion.BarraDeteccionStatus==BUSY_WAIT){
			    if(vfd.v.timer==0)
			    	Deteccion.BarraDeteccionStatus=0;}    
	      if(Deteccion.BarraDeteccionStatus==BUSY)
                goto maquinaEstados;//termina de graficar, correle.
	      else {if(vfd.bits.b.FIFOonReset)//vamos a cambiar  de menu y no podemos graficar nada
	    	          return;
	            else{vfd.f1.resetFIFOS(); 
	                 return;}}} //no hay menu pendiente, estado normal de graficacion
maquinaEstados:
      ...
      switch(estado){
    	  case 0: if(vfd.f1.pop(x,y,p)){ //No esta vacio
    		           ...
    		           vfd.v.index=0;
    	               Deteccion.BarraDeteccionStatus=BUSY;}
    		      break;
    	  case 1:switch(*p){
					  case 	_BOX_:*box1=*x;estado++;break;
					  case _CHAR_:estado=3;break;
					  case _PUNTO_:if(dds.Bnderas.bit.EventDDS==FALSE)//DDS1.Banderas.bit.EventDDS==FALSE)
						                  estado=34;
					               else estado=5;
						           break;
					  case _RAYA_:break;
					  case _BOLD_://__asm(nop);
					              //__asm(Halt);
						          estado=34;
					              break;//debug
					  case _POS_:estado=4;break;
					  case _DDS_BORRAR:estado=4;break;
					  case _DDS_reZOOM:estado=4;break;
    	  ...
    	  case 2:if(*box0>MAX_BOXES)
				 ...
				 estado=33;//emitir los datos; FIN DE CAJAS
				 break;//fin case 2------------------------------------
    	  case 3:vfd.v.dat[0]=*x; //x=vfd.v.dat[13];y=vfd.v.dat[12];p=vfd.v.dat[11];			 
			     vfd.v.nbytes=1;//bytes a emitir
                 estado=33;
                 break;//fin de char
    	  case 4:vfd.v.dat[0]=0x1F;//INICIA COMANDO DE POSICION
				 vfd.v.dat[1]=0x24;
    	  ...
    	  case 5:if(dds.Bnderas.bit.Apagando==TRUE){//se esta apagando la DDS para ir a otro lado
    		            dds.Bnderas.bit.EventDDS=FALSE;//DDS1.Banderas.bit.EventDDS=FALSE;//se apreto la parte borrar DDS no y no se autoriza pintar otro punto y aqui esperamos que termine de pintar el punto que esta pintando
						vfd.f1.resetFIFOS();
						Deteccion.CuadroMadreReady=FALSE;//ya no genere cuadros
						dds.Bnderas.bit.Apagando=FALSE;
    	                estado=34;break;}//Ya se apago el DDS1
    	         vfd.v.dat[0]=0x1F;
    	         vfd.v.dat[1]=0x28;
    	         vfd.v.dat[2]=0x64;
    	         vfd.v.dat[3]=0x10;
    	  ...
    	  case 6:vfd.v.timer=4; //FIN DE DESPLGUE DEL MENU, INDICA QUE SE DESPLIEGO UN MENU EN LA vfd	      
    	    	         estado++;break;
    	  ...
    	  case 33:if(vfd.v.nbytes==vfd.v.index){
    		            ...
    		            vfd.v.timer=DELAY_TIME;
    	                break;} 
    		      if(vfd.v.timer==0){//descontado por timer         
    		          VFDserial_SendChar(vfd.v.dat[vfd.v.index]);
     		          vfd.v.dat[vfd.v.index++]=0; 
     		          switch(menu.contexto.Actual){
     		        	  case PANTALLA_DDS:vfd.v.timer=3;break;
     		        	  default:vfd.v.timer=5;break;}}
    		      break;//fin de enviar el Buffer
    	
    	  case 34://esperamos que lleguen los ultimos datos al display 
    		      if(vfd.v.timer==0){
    		    	  Deteccion.BarraDeteccionStatus=BUSY_WAIT;//terminamos de graficar algo.      
    		    	  vfd.v.timer=BUSY_K;
    		    	  cleanArray(&vfd.v.dat[0],DATOS_SIZE,0);
    		    	  estado=0;}
    		      break;
    	  default:estado=0;break;}//fin estado principal-----------------------------------------      
