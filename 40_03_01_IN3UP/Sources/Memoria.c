...
//			 PasswordLevel2[0]=0;}
   ...
   Deteccion.Altura=60;
   Deteccion.Sensibilidad=130;
   Deteccion.Phase=30.7;
   dds.Bnderas.bit.Aspect_Zoom=0;
...
uword word16;    //disponemos de 128kbytes
float4 word32;    
//Asisgnacion de VAriables de Producto POrPRIMERA VEZ A ESPACIO CACHE
                        ...
                        *(p+23) =BorrarContadores=APAGAR;
                        word32.f=Deteccion.Phase=25.1;
                        *(p+24) =word32.byte[0];     
                        *(p+25) =word32.byte[1];
                        *(p+26) =word32.byte[2];
                        *(p+27) =word32.byte[3];
                        
                        *(p+28) =frecuenciaSeleccion=MEDIA;                           //1EH
                        *(p+29) =MarcarAltura='0';                      //1FH
                        *(p+30) =GananciaDDS='0'; 
                        *(p+31) =zoom.Zoom=99;               //29H    
                        *(p+32) =cuadritoDDS='0';             //2AH  
                         word16.wordx=Deteccion.Sensibilidad=25;// OOM_MAX_Y_99;
                         *(p+33)=word16.byte[0];
                         *(p+34)=word16.byte[1];
                         word16.wordx=Altura=32000;
                         *(p+35)=word16.byte[0];
                         *(p+36)=word16.byte[1];
//                         word32.wordx=frecuency=1000000;                //MERGE variable 32bits frecuency    
                         *(p+37)=word16.byte[0];
                         *(p+38)=word16.byte[1];
                         word16.wordx=PulsosTiempoRechazo=15;
                         *(p+39)=word16.byte[0];
                         *(p+40)=word16.byte[1];
                         word16.wordx=PulsosTiempoEspera=30;//rechazo espera de rechazo
                         *(p+41)=word16.byte[0];
                         *(p+42)=word16.byte[1];
                         *(p+43)=Ganancia=ALTA;
...
//Asisgnacion de VAriables de Producto POrPRIMERA VEZ A ESPACIO CACHE
                        ...
                        *(p+31) =zoom.Zoom=99;               //29H    
                        *(p+32) =cuadritoDDS='0';             //2AH  
                         word16.wordx=Deteccion.Sensibilidad=ZOOM_MAX_Y_99;
                         *(p+33)=word16.byte[0];
                         *(p+34)=word16.byte[1];
                         word16.wordx=Altura=31234;
...
float4 word32;
       ...
       word32.byte[2]     =*(p+26);
       word32.byte[3]     =*(p+27);
       Deteccion.Phase    =word32.f;
      
      // frecuenciaSeleccion=*(p+28);                           //1EH
       MarcarAltura       =*(p+29);                   //1FH
       GananciaDDS        =*(p+30); 
       zoom.Zoom          =*(p+31);              //29H  
       cuadritoDDS        =*(p+32);             //2AH  
       word16.byte[0]     =*(p+33);
       word16.byte[1]     =*(p+34);
       Deteccion.Sensibilidad=word16.wordx;
       word16.byte[0]     =*(p+35);
       word16.byte[1]     =*(p+36);
       Altura=word16.wordx;
       word16.byte[0]     =*(p+37);
...
float4 word32;
              ...
              //  *(p+23) //=BorrarContadores; libre se paso a varible de sistema
                word32.f=Deteccion.Phase;
                *(p+24)=word32.byte[0];
                *(p+25)=word32.byte[1];
                *(p+26)=word32.byte[2];
                *(p+27)=word32.byte[3];
                *(p+28) =frecuenciaSeleccion;                           //1EH
                *(p+29) =MarcarAltura;                      //1FH
                *(p+30) =GananciaDDS; 
                *(p+31) =zoom.Zoom;               //29H   
                *(p+32) =cuadritoDDS;             //2AH  
                 word16.wordx=Deteccion.Sensibilidad;
                *(p+33)=word16.byte[0];
                *(p+34)=word16.byte[1];
                 word16.wordx=Altura;
                *(p+35)=word16.byte[0];
...
//                    		  writeEEPROMbyte2(ESPACIO_C_ADD+24,Deteccion.phase);}
                          if(arg2==RESET){//se quiere resetear los contadores 
                        	  ...
                        	  writeNVRAMblock(ESPACIO_E_ADD,4,p);//borrar 4 byteDeteccion.s de amboDeteccion.s contadores
                              break;}}
                      break;
       ...
       case PARAMETROS_DE_DETECCION://1<Altura<32000, MarcarAtura=No|Si
                      if(operacion==RESTAURAR){
                         ...
                         //word32.byte[2]=*(p+2);
                         //word32.byte[3]=*(p+3);
                         Altura=word16.wordx;    //deprecated
                         MarcarAltura=*(p+4);
                         I2CliberarModulo();
                         p=readEEPROMblock(add,FRECUENCIA_SELECT_SIZE);
                         frecuenciaSeleccion=*p;
 ...
 * las direcciones son char las 2 porque el display es 512x127=ventala de 192x127
 ...
 * del DDS, status actual en el zoom actual.
   ...
   x: valor de pixelDDS de 0 a 192 ó de 0 a 127, son coord de pantalla centro =Cx,Cy.
...
//executar el zoom
void Memoria_Pixeles_DDS(signed short int x,signed short int y,unsigned char ux,unsigned char uy){

unsigned char reg,bit;
//unsigned char xr;
   if(vfd.bits.b.DDSon){// dds.Bnderas.bit.EventDDS){
	   if((x==0)&&(y==0))
			   return;
	   if(is_Zooms_out_Limits(x,y)){//los valores ssint estan fuera de limite de este zoom?
		     dds.pix.f.append(x,y);
	         return;}
	   bit=getRegistro_X_bit(&reg,ux);//que registro y bit es la coordenada x     
	   if((dds.pix.pix[reg][uy]&bit)!=bit){//esta activasdo el bit que representa este pixel?
		   vfd.f1.append(ux,uy,_PUNTO_);//no esta guardado, entos hay que graficarlo
	       dds.pix.pix[reg][uy]|=bit;}//no esta guardado, tons se guardda
       }//fin bandera
}//fin de memoria DDS-9 -----------------------------------
	
void reset_Memo_Pix_DDS(void){
unsigned char i,j;	
	for(i=0;i<24;i++)
     for(j=0;j<128;j++)
	   dds.pix.pix[i][j]=0;
}//fin reset_Memo_Pix_DDS--------------------------------------------



unsigned char is_Zooms_out_Limits(signed short int x,signed short y){
	  if((absSSInt(x)>zoom.Maxx)||(absSSInt(y)>zoom.Ymax))
	               return TRUE;
	  else return FALSE;
}//fin is_Zooms_out_Limits---------------------------------------------
...
/* se le ingresa valores en el rango DDS
 *  para graficar los puntos. coordenadas DDS ya centradas y escaladas
 *  si no esta guadado regresa */
...
//    
//
//    convert_sint_to_DDS(x,)
//    bit=getRegistro_X_bit(&reg,x);//que registro y bit es     
//    
//
//    for(i=0;i<SIZE1;i++){
//    	if(bufferX[i]==x)
...
void Controlador_Driver_de_EEPROM(void){
	
	
}//fin Controlador_Driver_de_EEPROM----------------------------
