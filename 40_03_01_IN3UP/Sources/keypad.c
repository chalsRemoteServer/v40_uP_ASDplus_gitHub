...
static unsigned short int count;
 if(menu.b.b.MenuPendiente){
	 wait=1;
	 return;}
 else{if(wait){//espera un rato despues deque cambiemos de menu
	    if(count--==0){
	    	count=455;wait=0;return;}}}
 if(keypad.b.enable){//bit que indica si esta habilitado el keypad
	...
	if(keypad.F.ncount>15){//hay datos en la FIFO? keypad.F.append
       ...
       switch(c){//automata: char anterior es C  <<-----manejo del char anterior
			...
			case 'A': //entradas y manage de Entradas del automata 
			case 'L'://ESTOS SON LOS: IGUALES
			case 'E'://Siemmpre y cuando coincidan con el que acaba de llegar
			case 'R':switch(menu.contexto.Actual){        //Sin Anomalias      Con Anomalias
				        case TEXT_PROCESSOR:filtro=1;break;//135,
				        case MENU_INSIGHT:  filtro=1;break;//estaba en 100 y se atrasa
				        case AJUSTE_PARAMETRICO_DE_PRODUCTO:filtro=1;break;
				        default:filtro=1;break; }//                                95
				     if(*c0==c)//El char apretado es igual al que llego 
				          k='D';//El caracter que tenemos el el mismo que se apreto orita 
				     else k='I';//el char que tenemos es diferente al que llego
			         break;
			case '*':k='*';break;         
	   ...
	   switch(*s){//Inicia el AUTOMATA
		  ...
		  case 4:switch(k){
		   ...
		   case 7:switch(k){
				   case '*':
				   case 'C':
				   default:if(*c0!=keypad.v.c)
					            pre_menus(keypad.v.c0);
				           else pre_menus(keypad.v.c); break;}
		          break;
