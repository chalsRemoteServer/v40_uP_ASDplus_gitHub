...
//VERSION 3. ACTIVA  las Banderas para que se dispare el sistema operativo
/*  El cambio de contexto*/
/*  unsigned char destino,//al que quiero ir
	unsigned char actual,//en el que estoy
	unsigned char padre,//de donde vengo
	...
	*/
void cambio_de_contexto(unsigned char destino){//displayControladorPantalla}
const unsigned char SIN_PASSWORD=0xF9;
const unsigned char SIN_CAMBIOS =0xF8;
unsigned char status;
	menu.b.b.MenuPendiente=TRUE;
	//menu.contexto.control=0;
	...
	switch(destino){//++++++++++++NO MOVER EL ORDEN DE LOS CASES PORQUE AFECTARIA LAS AUTRIZACION DE DE PERMISOS ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++PASO-1+++++1					              
		case PANTALLA_DDS:
				switch(menu.contexto.Actual){
					case MENU_INSIGHT:menu.b.b.MenuPendiente=TRUE;
									  menu.contexto.Anterior=MENU_INSIGHT;
									  menu.contexto.Modificado=0;
									  //menu.contexto.control=0;
									  menu.contexto.final=destino;
									  menu.contexto.padre=PORTAL_INICIO;break;
					case AJUSTE_PARAMETRICO_DE_PRODUCTO:
			                	        menu.b.b.MenuPendiente=TRUE;
										menu.contexto.Anterior=AJUSTE_PARAMETRICO_DE_PRODUCTO;
										menu.contexto.Modificado=0;
										menu.contexto.control=SERVICIO;//guarda Permisos
										menu.contexto.final=destino;
										menu.contexto.padre=AJUSTE_PARAMETRICO_DE_PRODUCTO;
							            break;
					case PANTALLA_DDS:menu.b.b.MenuPendiente=TRUE;
							menu.contexto.Anterior=AJUSTE_PARAMETRICO_DE_PRODUCTO;
							menu.contexto.Modificado=PANTALLA_DDS;
							menu.contexto.control=menu.contexto.control;//guarda Permisos
							menu.contexto.final=PANTALLA_DDS;
							menu.contexto.padre=AJUSTE_PARAMETRICO_DE_PRODUCTO;
							break;
					default:status=SIN_CAMBIOS;break;}
		     break;//fin PANTALLA DDS------------------------------------------
		case AJUSTE_PRODUCTO_SEL:switch(menu.contexto.control){
									case AJUSTE_DE_PRODUCTO:
										   if(menu.contexto.Anterior==AJUSTE_DE_PRODUCTO){
											    menu.b.b.MenuPendiente=TRUE;
											    menu.contexto.padre=AJUSTE_DE_PRODUCTO;//de donde provengo
												menu.contexto.Anterior=AJUSTE_DE_PRODUCTO;//de donde vengo, que eligimos
												menu.contexto.Modificado=0;
												menu.contexto.control=SERVICIO;
												menu.contexto.final=destino;}             
									default:break;}//fin switch menu.contexto.control
			                     break;//fin case AJUSTE_PRODUCTO_SEL
		case TEXT_PROCESSOR:switch(menu.contexto.final){
								case NUEVO_PRODUCTO:
									    menu.b.b.MenuPendiente=TRUE;
										menu.contexto.Anterior=AJUSTE_DE_PRODUCTO;
										menu.contexto.Modificado=0;
										menu.contexto.control=SERVICIO;
										menu.contexto.final=AJUSTE_DE_PRODUCTO_MODE;
										menu.contexto.padre=PORTAL_INICIO;
										break;
										
								default:break;}
		case AJUSTE_DE_PRODUCTO_MODE:switch(menu.contexto.Actual){
											case AJUSTE_PRODUCTO_SEL:
												  switch(menu.contexto.control){
													  case AJUSTE_DE_PRODUCTO:
														    ...
														    break;//fin case ajuste de prod.        
													  default:break;}//fin control
												  break;//fin AJUSTE PRODUCTO SEL;
											case NUEVO_PRODUCTO:
												menu.b.b.MenuPendiente=TRUE;
												...
												menu.contexto.control=SERVICIO;
												menu.contexto.final=AJUSTE_DE_PRODUCTO_MODE;
												menu.contexto.padre=ESTAS_SEGURO;
												break;
											case TEXT_PROCESSOR:
...
//															//menu.contexto.control=0;
															...
															menu.contexto.padre=ESTAS_SEGURO;}
													break;
											default:break;}//fin 2nd switch
			                         break;//fin case ajuste prod Mode
		case ESTAS_SEGURO:switch(menu.contexto.final){
			                    case SELECCIONAR_PRODUCTO:
			                    	    if(menu.contexto.Anterior==AJUSTE_DE_PRODUCTO){
												menu.b.b.MenuPendiente=TRUE;
												menu.contexto.Anterior=AJUSTE_DE_PRODUCTO;
												menu.contexto.Modificado=ESTAS_SEGURO;
//												//menu.contexto.control=0;
												menu.contexto.final=SELECCIONAR_PRODUCTO;
												menu.contexto.padre=PORTAL_INICIO;}
										break;
			                    case AJUSTES_AVANZADOS://depende de la opcion si o no para saber a que menu nos vamos
			                    	  if((menu.contexto.padre==NUEVO_PRODUCTO)&&
			                    	      (menu.contexto.Anterior==AJUSTE_DE_PRODUCTO_MODE)){
			                    		  	  	menu.b.b.MenuPendiente=TRUE;
												menu.contexto.Anterior=NUEVO_PRODUCTO;
												menu.contexto.Modificado=ESTAS_SEGURO;
												menu.contexto.control=AJUSTE_PARAMETRICO_DE_PRODUCTO;
												menu.contexto.final=AJUSTES_AVANZADOS;//AJUSTE_PARAMETRICO_DE_PRODUCTO;
												menu.contexto.padre=AJUSTE_DE_PRODUCTO;}
			                    	  break;//del case Ajuistes Avanzados              
			                    default:break;}
		                    break; 
		case PORTAL_INICIO: menu.b.b.MenuPendiente=TRUE;
							menu.contexto.Anterior=PORTAL_INICIO;
							menu.contexto.Modificado=0;
							//menu.contexto.control=0;
							menu.contexto.final=destino;
							menu.contexto.padre=PORTAL_INICIO;
		...
		case AJUSTE_DE_PRODUCTO:
			     switch(menu.contexto.Actual){
			      ...
			      case AJUSTE_PRODUCTO_SEL: if(menu.contexto.control==AJUSTE_DE_PRODUCTO){
												menu.b.b.MenuPendiente=TRUE;
												menu.contexto.Anterior=PORTAL_INICIO;
												menu.contexto.Modificado=0;
//												//menu.contexto.control=0;
												menu.contexto.final=destino;
												menu.contexto.padre=PORTAL_INICIO;}
			                                 break;//fin case product sel
			       case MENU_INSIGHT:menu.b.b.MenuPendiente=TRUE;
									 menu.contexto.Anterior=MENU_INSIGHT;
									 menu.contexto.Modificado=0;
									 menu.contexto.control=TEXT_PROCESSOR;
									 menu.contexto.final=destino;
									 menu.contexto.padre=PORTAL_INICIO;
									 menu.contexto.permisos=0;
			    	                 break;
			       default:break;}//fin switchc  switch(menu.contexto.Actua
			     break;// case AJUSTE_DE_PRODUCTO---
		case AJUSTE_PARAMETRICO_DE_PRODUCTO:
			   if((menu.contexto.Anterior==NUEVO_PRODUCTO)&&
				 (menu.contexto.padre==AJUSTE_DE_PRODUCTO)&&	   
				 (menu.contexto.Modificado==AJUSTES_AVANZADOS)){
						...
						//menu.contexto.control=0;
						menu.contexto.final=destino;
						menu.contexto.padre=NUEVO_PRODUCTO;}	  
			    else{
					menu.b.b.MenuPendiente=TRUE;
					menu.contexto.Anterior=MENU_INSIGHT;
					menu.contexto.Modificado=0;
					...
					menu.contexto.final=destino;
					menu.contexto.padre=PORTAL_INICIO;}
			    break;//fin case ajustes parametricos de producto   
		case AJUSTE_DE_SISTEMA:
							...
							menu.contexto.padre=PORTAL_INICIO;
							break;
		case MENU_ADMINISTRATIVO://AJUSTE_DE_SISTEMA				
		case AJUSTE_DE_VIBRACION:
		case CONFIGURAR_ENTRADAS_DE_SISTEMA:
		case ID_COMUNICACIONES:
		case INGENIERIA_GRACIDA:
							menu.b.b.MenuPendiente=TRUE;
							menu.contexto.Anterior=AJUSTE_DE_SISTEMA;
							menu.contexto.Modificado=ESTAS_SEGURO;
							//menu.contexto.control=0;
							menu.contexto.final=destino;
							menu.contexto.padre=PORTAL_INICIO;
							break;
		case ENTRADA_TACO://CONFIGURAR_ENTRADAS_DE_SISTEMA:
		...
		case ENTRADA_AUX2:
						menu.b.b.MenuPendiente=TRUE;
						menu.contexto.Anterior=CONFIGURAR_ENTRADAS_DE_SISTEMA;
						menu.contexto.Modificado=ESTAS_SEGURO;
						//menu.contexto.control=0;
						menu.contexto.final=destino;
						menu.contexto.padre=CONFIGURAR_ENTRADAS_DE_SISTEMA;
						break;
							
		case CONTROL_PASSWORD:   //MENU_ADMINISTRATIVO:				
		...
		case CONTROL_USER:  
						menu.b.b.MenuPendiente=TRUE;
						menu.contexto.Anterior=MENU_ADMINISTRATIVO;
						menu.contexto.Modificado=0;//NO PREGUNTA SI GUARDA solo guarda
						//menu.contexto.control=0;
						menu.contexto.final=destino;
						menu.contexto.padre=MENU_ADMINISTRATIVO;
						break;
		case TIPO_DE_MAQUINA://INGENIERIA_GRACIDA
		case GANANCIA_:
	    case FILTRO_:	                         
	    case ID_MAQUINA:
	    case PARTE_11:
		case INGENIERIA_GRACIDA2:
	    				menu.b.b.MenuPendiente=TRUE;
						menu.contexto.Anterior=INGENIERIA_GRACIDA;
						menu.contexto.Modificado=ESTAS_SEGURO;
						//menu.contexto.control=0;
						menu.contexto.final=destino;
	    ...
	    case CONTROL_DE_FRECUENCIA:	
	    	            if(menu.contexto.Actual==AJUSTE_DE_SISTEMA){
	    	            	menu.b.b.MenuPendiente=TRUE;
							menu.contexto.Anterior=AJUSTE_DE_SISTEMA;
							menu.contexto.Modificado=ESTAS_SEGURO;
							//menu.contexto.control=0;
							menu.contexto.final=destino;
							menu.contexto.padre=AJUSTE_DE_SISTEMA;}	
	    	            else{if(menu.contexto.Actual==INGENIERIA_GRACIDA)
	    	            		menu.b.b.MenuPendiente=TRUE;
								menu.contexto.Anterior=INGENIERIA_GRACIDA;
								menu.contexto.Modificado=ESTAS_SEGURO;
								//menu.contexto.control=0;
								menu.contexto.final=CONTROL_DE_FRECUENCIA2;
								menu.contexto.padre=INGENIERIA_GRACIDA;}
	    	            break;
	
		case SELECCIONAR_PRODUCTO:	//AJUSTE DE PRODUCTO
		case NUEVO_PRODUCTO:	
		...
		case COPY_PRODUCTO:
								...
								menu.contexto.padre=PORTAL_INICIO;
						     break;
		case AJUSTES_AVANZADOS://AJUSTE_PARAMETRICO_DE_PRODUCTO
			                   if((menu.contexto.Actual==AJUSTE_DE_PRODUCTO_MODE)&&
			                       (menu.contexto.Anterior==NUEVO_PRODUCTO)){
			                	    menu.b.b.MenuPendiente=TRUE;
									menu.contexto.Anterior=AJUSTE_DE_PRODUCTO_MODE;
									menu.contexto.Modificado=0;
									//menu.contexto.control=0;
									menu.contexto.final=AJUSTES_AVANZADOS;
									menu.contexto.padre=NUEVO_PRODUCTO;break;}
			                   if((menu.contexto.padre==NUEVO_PRODUCTO)&&
			                	   ...
			                	   (menu.contexto.Anterior==AJUSTES_AVANZADOS)){
									...
									menu.contexto.padre=AJUSTE_PARAMETRICO_DE_PRODUCTO;}
	                            break;
		case NOMBRE_PRODUCTO://AJUSTES_AVANZADOS
			               if((menu.contexto.padre==NUEVO_PRODUCTO)&&
			                  (menu.contexto.Anterior==AJUSTE_DE_PRODUCTO_MODE)&&
			                  (menu.contexto.Actual==AJUSTES_AVANZADOS)){
			            	   	   	menu.b.b.MenuPendiente=TRUE;
									menu.contexto.Anterior=AJUSTES_AVANZADOS;
									menu.contexto.Modificado=AJUSTE_DE_PRODUCTO_MODE;
									menu.contexto.control=TEXT_PROCESSOR;
									menu.contexto.final=NOMBRE_PRODUCTO;
									menu.contexto.padre=NUEVO_PRODUCTO;}
			               else{	   
								menu.b.b.MenuPendiente=TRUE;
								menu.contexto.Anterior=AJUSTES_AVANZADOS;
								...
								//menu.contexto.control=0;
								menu.contexto.final=destino;
								menu.contexto.padre=AJUSTES_AVANZADOS;}
				            break;
		case MENU_INSIGHT:  menu.b.b.MenuPendiente=TRUE;
		...
		case AJUSTE_TIEMPO_RECHAZO:        
							...
							menu.contexto.padre=AJUSTES_AVANZADOS;
							break;
		case INIZIALIZAR_MEMO://INGENIERIA_GRACIDA2	
		case PRUEBA:
		case MEDIDAS_DE_VOLTAJES:
					menu.b.b.MenuPendiente=TRUE;
					menu.contexto.Anterior=INGENIERIA_GRACIDA2;
					menu.contexto.Modificado=ESTAS_SEGURO;
					//menu.contexto.control=0;
					menu.contexto.final=destino;
					menu.contexto.padre=INGENIERIA_GRACIDA2;
					break;
		case I2CMENU:
		case I2CEEPROMREAD:
		case I2CNVRAMDATA:
		...
		case PRUEBAS_ENTRADAS:					
			        menu.b.b.MenuPendiente=TRUE;
					menu.contexto.Anterior=PRUEBA;
					menu.contexto.Modificado=0;
					////menu.contexto.control=0;
					menu.contexto.final=destino;
					menu.contexto.padre=PRUEBA;
					break;
		case INFORMACION_DE_USUARIO:
					...
					menu.contexto.padre=MENU_INSIGHT;
					break;
		case PRUEBAS_RUIDO://INFORMACION_DE_USUARIO
		case MEDIDAS_DE_VOLTAJE:
		case AJUSTE_DE_PRODUCTOINFO:
		case TIEMPO_DE_RECHAZOINFO:
		case AJUSTE_SISTEMA_ENTRADAS:		
					menu.b.b.MenuPendiente=TRUE;
					menu.contexto.Anterior=INFORMACION_DE_USUARIO;
					menu.contexto.Modificado=0;
					//menu.contexto.control=0;
					menu.contexto.final=destino;
					menu.contexto.padre=INFORMACION_DE_USUARIO;
					break;
 ...
 if(status==SIN_CAMBIOS){
	 menu.contexto.final=destino;
	 menu.contexto.Anterior4=menu.contexto.Anterior3;
	 menu.contexto.Anterior3=menu.contexto.Anterior2;
	 menu.contexto.Anterior2=menu.contexto.Anterior1;
	 menu.contexto.Anterior1=menu.contexto.Anterior;
...
/* Cuando se quiere cambiar de contexto se llama no importa si hay en medio un guardar o una clave
 *  desde aqui lo gestionamos, es automatico no teneemos que indicar que va haber 
 *  cambio de nombre, clave o contexto guardar*/
void cambio_de_contexto_v2(unsigned char destino){//displayControladorPantalla}
const unsigned char GUARDAR_CAMBIOS=0xF9;
const unsigned char INGRESAR_CLAVE=0xF8;
const unsigned char SIN_CAMBIOS=0xF7;
unsigned char status;
	menu.b.b.MenuPendiente=TRUE;
	//menu.contexto.control=0;
	menu.contexto.Modificado=0;
				...
				switch(menu.contexto.Actual){
					case MENU_INSIGHT:menu.b.b.MenuPendiente=TRUE;
									  ...
									  menu.contexto.final=destino;
									  	 ...
									  	 menu.contexto.Anterior3=menu.contexto.Anterior2;
									  	 menu.contexto.Anterior2=menu.contexto.Anterior1;
									  	 menu.contexto.Anterior1=menu.contexto.Anterior0;
									  	 menu.contexto.Anterior0=menu.contexto.Anterior;
									  	 menu.contexto.Anterior =menu.contexto.Actual;
...
//	menu.b.b.semMenuPendiente=TRUE;
//	menu.contexto.Anterior=menu.contexto.Actual;
//	menu.contexto.Actual=destino;
// }//------------------------------------------------------------------------
...
//	pushFIFOcOP_inicioCMD();
	...
	if(Ganancia==ALTA){
		VFDserial_SendChar('A');
		VFDserial_SendChar('l');
		VFDserial_SendChar('t');
		VFDserial_SendChar('a');
		Ganancia=BAJA;}
	else{
		VFDserial_SendChar('B');
		VFDserial_SendChar('a');
		VFDserial_SendChar('j');
		VFDserial_SendChar('a');
	    Ganancia=ALTA;}
	//pushFIFOcOP_finCMD();
...
unsigned char r=0;	
	switch(menu.contexto.Actual){
	  case PARAMETROS_DE_DETECCION:
		          switch(*index){
		         	  case 0:VFDposicion(0,cursory);
					         r+=VFDserial_SendChar1('w');
					         r+=VFDserial_SendChar1('e');
					         r+=VFDserial_SendChar1('e');
					         r+=VFDserial_SendChar1('e');
					         r+=VFDserial_SendChar1('e');
					         r+=VFDserial_SendChar1('e');
					         r+=VFDserial_SendChar1('e');
					         r+=VFDserial_SendChar1('e');	    
					         r+=VFDserial_SendChar1(0x02);
					         r+=VFDserial_SendChar1(0x01);
					         if(r>0)
					        	 __asm(Halt);
					         break;
		        	  case 1:VFDposicion(0,cursory);
		   				     VFDserial_SendChar1('>');
			             	 break;
		        	  default:break;}
		          break;
...
unsigned char a[6];
			...
			VFDposicion((unsigned short int)(posInit-8),posy);
			VFDserial_SendChar1(a[b[0]]);
			VFDserial_SendChar1(a[b[1]]);
			VFDserial_SendChar1(a[b[2]]);
			VFDserial_SendChar1(a[b[3]]);
			VFDserial_SendChar1(a[b[4]]);
			VFDserial_SendChar1(a[b[5]]);
			//pushFIFOcOP_finCMD();
return;
}//FIN  display_UInt_CharsSelected-----------------------------------------------------------------------------
...
unsigned char i,size,index;	
 ...
 //void (*pVFDposicion)(unsigned short x,unsigned short y);
        ...
        for(i=1;i<10;i++)   //buscar el tamaño del array, para evitar un parametro
        	if(*(posiciones+i)==0){
        		 size=i;
        	     break;}
        for(i=0;i<size;i++){//verificar que la POSY este dentro del rango
        	if(*(posiciones+i)>POSY14){
        		__asm(nop);
        		__asm(Halt);
        	    __asm(nop);}}
			...
			if(*(posiciones+(index-1))==POSY0){//X0 vamos a subir a la POSY0
				if(menu.contexto.Actual==PANTALLA_DDS){
					...
					*posx=esqx-16;*posy=POSY0;
					...
					VFDserial_SendChar(0x01);//sin estos dos numeros no se pinta la X
				    }
				else{
					VFDposicion(*posx,*posy);
					VFDserial_SendChar(' ');
				...
				if(menu.contexto.Actual==PANTALLA_DDS){//DDS para subir
					if(*posy==POSY14){
						  ...
						  VFDserial_SendChar(0x01);}//delays
					else{ 
						VFDposicion(*posx,*posy);
						VFDserial_SendChar(' ');}
					*posy=*(posiciones+(index-1));
					VFDposicion(*posx,*posy);
					VFDserial_SendChar('>');
					VFDserial_SendChar(0x01);//sin este no se pinta el anterior
				    return;}
				else{
					VFDposicion(*posx,*posy);
					VFDserial_SendChar(' ');
				...
				*posx=esqx;
				if(menu.contexto.Actual==PANTALLA_DDS){
					...
					*posy=*(posiciones+(index+1));
					VFDposicion(*posx,*posy);
					VFDserial_SendChar('>');
					VFDserial_SendChar(0x01);//sin esto no se pinta el anterior
					return;}
				else{
					VFDposicion(*posx,*posy);
					VFDserial_SendChar(' ');
 ...
 * return si se modifico algo o falso si no se pudo modificar 
 *  MEMORIA 8 bytes */
unsigned char displayControlCursorSubMenusKey_v2(unsigned char direccion,
		unsigned char *posx, unsigned char *posy,unsigned char esqx,
		unsigned char *posiciones,unsigned char posx0,
		unsigned char *inst){//&cursorx,&cursory,POSXESQ235,&key[0]){
auto unsigned char i,size,index;	
auto unsigned char estado1,ret=0,*inst1;
//void (*pVFDposicion)(unsigned short x,unsigned short y);
...
size=*(inst+2);
index=*(inst+3);
inst1=inst+4;//4,5,6,7

 switch(estado1){
   case 1:for(i=1;i<10;i++) if(*(posiciones+i)==0){size=i;break;}//buscar el tamaño del array, para evitar un parametro
          estado1++;break;
   case 2:for(i=0;i<size;i++){//verificar que la POSY este dentro del rango
        	if(*(posiciones+i)>POSY14){estado1=100;break;}}
          estado1++;break;
   case 3:for(i=0;i<size;i++)//buscar en que posicion estoy
	        if(*(posiciones+i)==*posy){ //{POSY0,POSY2,POSY4,POSY6,POSY8,0};	
	    		index=i;  //en donde estoy actualmete 
	    		break;}
           if(direccion==PARRIBA) estado1=10;else estado1=50;break;
   case 10:if(*posy==POSY0){estado1=100;break;}estado1++;break;
   case 11:if(*(posiciones+(index-1))==POSY0){//X0 vamos a subir a la POSY0
	           if(menu.contexto.Actual==PANTALLA_DDS) estado1=20;
	           else  estado1=30;}
   	   	   else{if(menu.contexto.Actual==PANTALLA_DDS){//DDS para subir
   	   		        if(*posy==POSY14){estado1=40;}
   	   		        else{estado1=41;}}}
   case 20:if(VFDposicion(*posx,*posy))estado1++;break;
   case 21:if(VFDserial_SendChar1(' ')) estado1++;break;
   case 22:*posx=esqx-16;*posy=POSY0;  estado1++;break;
   case 23:if(VFDposicion(*posx,*posy))estado1++;break;
   case 24:if(VFDserial_SendChar1(' ')) estado1++;break; 
   case 25:if(VFDserial_SendChar1(' ')) estado1++;break;//X
   case 26:if(VFDserial_SendChar1('>')) estado1++;break;
   case 27:if(VFDserial_SendChar1('X')) estado1++;break;
   case 28:estado1=100;break;
   case 30:if(VFDposicion(*posx,*posy))estado1++;break;
   case 31:if(VFDserial_SendChar1(' '))estado1++;break;
   case 32:*posx=esqx;*posy=POSY0;estado1++;break;
   case 33:if(VFDposicion(*posx,*posy))estado1++;break;
   case 34:if(VFDserial_SendChar1('>'))estado1++;break;
   case 35:if(VFDserial_SendChar1('X'))estado1++;break;
   case 36:estado1=100;break;
   case 40:if(display_B_bold_DDS_v2(FALSE,inst1)) estado1=43;break;
   case 41:if(VFDposicion(*posx,*posy)) estado1++;break;
   case 42:if(VFDserial_SendChar1(' '))estado1++;break;
   case 43:*posy=*(posiciones+(index-1));estado1++;break;
   case 44:if(VFDposicion(*posx,*posy))estado1++;break;
   case 45:if(VFDserial_SendChar1('>')){estado1=100;break;}
   	   	   	break;
   case 50:if(*posy==POSY0) estado1++;
   	   	    else estado1=70;//ADSF;
   	   	   break;
   case 51:*posx=esqx;estado1++;break;
   case 52:if(menu.contexto.Actual==PANTALLA_DDS)estado1++;
            else estado1=60;//6tw
   case 53:if(VFDposicion(*posx,*posy))estado1++;break;
   case 54:if(VFDserial_SendChar1(' '))estado1++;break;
   case 55:if(VFDserial_SendChar1('x'))estado1++;break;
   case 56:*posx=posx0;*posy=*(posiciones+(index+1));
    		estado1++;break;
   case 57:if(VFDposicion(*posx,*posy))estado1++;break;
   case 58:if(VFDserial_SendChar1('>'))estado1++;break;
   case 59:estado1=100;break;
   case 60:if(VFDposicion(*posx,*posy))estado1++;break;
   case 61:if(VFDserial_SendChar1(' '))estado1++;break;
   case 62:if(VFDserial_SendChar1('x'))estado1++;break;
   case 63:*posx=posx0;*posy=*(posiciones+(index+1));
   	   	   	estado1++;break;
   case 64:if(VFDposicion(*posx,*posy))estado1++;break;
   case 65:if(VFDserial_SendChar1('>'))estado1++;break;
   case 66:estado1=100;break;
   case 70:if(index==(size-1)){//estamos en el ultimo POSY?
	   	   	   	 estado1=100;break; }
            else{estado1++;}break;
   case 71:if(menu.contexto.Actual==PANTALLA_DDS)
	              estado1++;
   	   	   	else estado1=78;///d678g
   	   	   	break;
   case 72:if(VFDposicion(*posx,*posy))estado1++;break;
   case 73:if(VFDserial_SendChar1(' '))estado1++;break;
   case 74:*posy=*(posiciones+(index+1));
   	   	    if(*posy==POSY14) estado1++;else {estado1=76;}
   	   	    break;
   case 75:if(display_B_bold_DDS_v2(TRUE,inst1)){estado1=100;break;}
   case 76:if(VFDposicion(*posx,*posy))estado1++;break;
   case 77:if(VFDserial_SendChar1('>')){estado1=100;break;}
   case 78:if(VFDposicion(*posx,*posy))estado1++;break;
   case 79:if(VFDserial_SendChar1(' ')) estado1++;break;
   case 80:*posy=*(posiciones+(index+1));estado1++;break;
   case 81:if(VFDposicion(*posx,*posy))estado1++;break;
   case 254:if(VFDserial_SendChar1('>')){estado1=100;break;}
   case 300:ret=TRUE;estado1=0;break;
   default:estado1=0;break;}//fin de switch------------------------
 *inst=estado1;
 *(inst+1)=i;
 *(inst+2)=size;
 *(inst+3)=index; 
...
unsigned char  n,m,c;
	...
	VFDposicion(DDS_MENU_X,POSY14);
	VFDserial_SendChar(c);
	VFDserial_SendChar('B');
...
auto unsigned char  n,m,c;
auto unsigned char estado9,ret;
 ...
 switch(estado9){
	 case 1:if(disp==TRUE){
		       n=TRUE;m=FALSE;c='>';}
	  	  	...
	  	  	estado9++;break;
	 case 2:if(VFDcommand_Bold_DDS_v2(n))estado9++;break;
	 case 3:if(VFDposicion(DDS_MENU_X,POSY14))estado9++;break;
	 case 4:if(VFDserial_SendChar1(c))  estado9++;break;
	 case 5:if(VFDserial_SendChar('B')) estado9++;break;
	 case 6:if(VFDcommand_Bold_DDS_v2(m)) estado9++;break;
...
//void display_Linea_10_DDS(unsigned char DDSaux,unsigned char inst,unsigned char inst2){
//unsigned char m[]="Z ";
//unsigned char m0[]=">Z ";
...
word n;
    ...
    switch(estado){
    	case 1:if(DDSaux==POSY10) estado++;
    	       else estado=4;
    	       break;
    	case 2:if(VFDposicion(POSXDDSCURSORMENU,y[5]))
    		        estado++;
    	        break;
    	case 3:if(VFDserial_SendBlock1(&m0[0],sizeof(m),&n,inst+1))
    		        estado=6;
    	        break;
    	case 4:if(VFDposicion(x5,y[5]))
    		       estado++;
    	 	   break;
    	case 5:if(VFDserial_SendBlock1(&m[0],sizeof(m),&n,inst+1))
    		       estado++;
    	       break;
...
unsigned char z[3];	
	     ...
	     //VFDserial_SendChar('¡');
	     //VFDserial_SendChar('°');
	     VFDserial_SendChar(z[1]);
	     VFDserial_SendChar(z[2]);
	     VFDserial_SendChar(0x01);
	     //VFDserial_SendChar('$');
	     //VFDserial_SendChar('#');
...
void  display_Sensibilidad_Limites(float angulo, unsigned short int sens) {
	//signed short int cx=96,cy=-64;
	//signed short int cxMax=192;
	//unsigned char v[4];
	float Angulo_Esquina = 33.69;//angulo en la esquina con pendiente positiva
	float Angulo_Esquina2 = 146.6;
	float m;//calculo perpendicular
//	int v;
	struct Coordsf P34, P67, P58, P910,Pab,Pcd;
	struct Coordf P1;//puntos de interseccion P2 y P3
	struct Ec_Recta fx, gx, hx;//funcinoes f(x) y g(x) y h(x)
	struct Ec_Recta px,A1,A2;//funciones p(x)
	struct Coordsuc vfx, vgx, vhx,vA1,vA2; //funciones VFD de f(x) y g(x)
//	struct Coordsuc vpx; //vpx es VFD  p(x) perpendicular
	//VFDdrawLine(1,192,0,0,127);//CX+64,CY-CY,CX-CX,CY+CY-1);
    float angi; //angulo de la directriz perpendicular paralela a las tapas de altura
	P1.x = 96; P1.y = -64;
	fx=getEcuacion_Recta(P1,angulo);
	P34=get_Puntos(fx,angulo,CENTRAL,Deteccion.Sensibilidad);
	P58 = get_Puntos_Perpendiculares(fx.m, P1, sens);
	gx=getEcuacion_Recta(P58.Pn,angulo);
 	P67=get_Puntos(gx,angulo,PARRIBA,Deteccion.Sensibilidad);
	hx=getEcuacion_Recta(P58.Pm,angulo);
	P910=get_Puntos(hx,angulo,PABAJO,Deteccion.Sensibilidad);
	
	  m =calculo_pendiente_inversa(angulo);//directriz de la Altura.
	px  =getEcuacion_Recta_Punto_Pendiente(P1,m);//ec. perpendicular ala directriz
	Pab =busqueda_Puntos_Altura(fx,sens);//puntos en directriz inversa donde cruza la altura.
	angi=get_angulo_Perpendicular(m,angulo);//angulo de la perpendicular
	 A1 =getEcuacion_Recta(Pab.Pm,angi);//ecuacion de tapa Altura
	 A2 =getEcuacion_Recta(Pab.Pn,angi);//get ec de recta de la tapa  Altura,perp  a directriz
	 
	Pab.Pm=get_Puntos_de_Interseccion(gx,A1); //PUNTO interseccion Altura y sensibilidad	
	Pab.Pn=get_Puntos_de_Interseccion(hx,A1);
	Pcd.Pm=get_Puntos_de_Interseccion(gx,A2); //PUNTO interseccion Altura y sensibilidad	
	Pcd.Pn=get_Puntos_de_Interseccion(hx,A2);
	
		
	
	
	vfx = traducirCoordenadas_to_VFD(P34/*,fx*/);//puntos P3 y P4
	vgx = traducirCoordenadas_to_VFD(P67/*,gx*/); 
	vhx = traducirCoordenadas_to_VFD(P910/*,hx*/);
	vA1 = traducirCoordenadas_to_VFD(Pab/*,A1*/);
	vA2 = traducirCoordenadas_to_VFD(Pcd/*,A2*/);		
	
	

	VFDdrawLine(1, vfx.Pa.x, vfx.Pa.y, vfx.Pb.x, vfx.Pb.y);
	VFDdrawLine(1, vgx.Pa.x, vgx.Pa.y, vgx.Pb.x, vgx.Pb.y);
	VFDdrawLine(1, vhx.Pa.x, vhx.Pa.y, vhx.Pb.x, vhx.Pb.y);
	VFDdrawLine(1, vA1.Pa.x, vA1.Pa.y, vA1.Pb.x, vA1.Pb.y);
	VFDdrawLine(1, vA2.Pa.x, vA2.Pa.y, vA2.Pb.x, vA2.Pb.y);
	__asm(nop);
	return;
	
	
//	if (angulo == Angulo_Esquina) {
//		P1.x = 96; P1.y = -64;
//		P34 = AgetEcuation(&fx, P1, angulo);
//		P58 = get_Puntos_Perpendiculares(fx.m, P1, sens);
//		P67 = AgetEcuation(&gx, P58.Pn, angulo);//recta inferior, Pn puntos inferiores
//		P67 = get_Evaluar_(P67, gx);//sacar ec.recta de linea inf.
//		P910 = AgetEcuation(&hx, P58.Pm, angulo);//recta superior, Pm puntos superiores
//		P910 = get_Evaluar_(P910, hx);//sacar la ec. recta linea sup.
//		return;
//	}
//	if (angulo < 90) {//para angulo 33.69,
//			P1.x = 96; P1.y = -64;
//			P34 = AgetEcuation(&fx, P1, angulo);//Ecuación central 
//			P34 = get_Evaluar_(P34, fx);
//			P58 = get_Puntos_Perpendiculares(fx.m, P1, sens);
//			P67 = AgetEcuation(&gx, P58.Pn, angulo);//recta inferior, Pn puntos inferiores
//			P67 = get_Evaluar_(P67, gx);//sacar ec.recta de linea inf.
//			P910 = AgetEcuation(&hx, P58.Pm, angulo);//recta superior, Pm puntos superiores
...
//		}//fin angulo----------------------------------------------
//		else {
//			if ((angulo > 90) && (angulo < Angulo_Esquina2)) {
//				P1.x = 96; P1.y = -64;
//				P34 = AgetEcuation2(&fx, P1, angulo);//Ecuación central 
//				P34 = get_Evaluar_2(P34,fx,angulo);
//				P58 = get_Puntos_Perpendiculares(fx.m, P1, sens);
//				P67 = AgetEcuation2(&gx, P58.Pn, angulo);//recta inferior, Pn puntos inferiores
//				P67 = get_Evaluar_2(P67,gx,angulo);//sacar ec.recta de linea inf.
//				P910 = AgetEcuation2(&hx, P58.Pm, angulo);//recta superior, Pm puntos superiores
//				P910 = get_Evaluar_2(P910,hx,angulo);
//				}
//			else {
...
//					P34 = get_Evaluar_2(P34,fx,angulo);
//					P58 = get_Puntos_Perpendiculares(fx.m, P1, sens);
//					P67 = AgetEcuation3(&gx,fx,P58.Pm,angulo);
//					P67 = get_Evaluar_2(P67,gx,angulo);
//					P910 = AgetEcuation3(&hx,fx,P58.Pn,angulo);
//					P910 = get_Evaluar_2(P910,hx,angulo);}
//				else { if (angulo == 90) {
//					P34.Pm.x = 96; P34.Pm.y = 0;
//					P34.Pn.x = 96; P34.Pn.y = -127;
//					P67.Pm.x = 96+sens; P67.Pm.y =0;
//					P67.Pn.x = 96 + sens; P67.Pn.y =-127;
//					P910.Pm.x = 96 - sens; P910.Pm.y = 0;
//					P910.Pn.x = 96 - sens; P910.Pn.y =-127;
//
//					fx = getEcuation(P34.Pm, P34.Pn);
//					gx = getEcuation(P67.Pm, P67.Pn);
//					hx = getEcuation(P910.Pm, P910.Pn);				
//				
//			     }}}}//fin elses
//
//	vfx = traducirCoordenadas_to_VFD(P34);//puntos P3 y P4
//	vgx = traducirCoordenadas_to_VFD(P67);
//	vhx = traducirCoordenadas_to_VFD(P910);
//
//	VFDdrawLine(1, vfx.Pa.x, vfx.Pa.y, vfx.Pb.x, vfx.Pb.y);
//	VFDdrawLine(1, vgx.Pa.x, vgx.Pa.y, vgx.Pb.x, vgx.Pb.y);
//	VFDdrawLine(1, vhx.Pa.x, vhx.Pa.y, vhx.Pb.x, vhx.Pb.y);
...
}//fin display sensibilidad raya de limites---------------------------


/* 17+21+8:MEMORIA BYTES   4:usi   17+24:float */
unsigned char  display_Sens_Lim_v2(float angulo,unsigned short int sens,
		                           unsigned char *inst,unsigned short int *usi,
		                           float *f){
float Angulo_Esquina = 33.69;//angulo en la esquina con pendiente positiva
float Angulo_Esquina2 = 146.6;
auto float m;//calculo perpendicular
auto float angi; //angulo de la directriz perpendicular paralela a las tapas de altura
auto unsigned char estado2,ret=0;
...
struct Ec_Recta px,A1,A2;//funciones p(x)
struct Coordsuc vfx,vgx,vhx,vA1,vA2; //funciones VFD de f(x) y g(x)
struct Coordsuc vgx0,vhx0;
const unsigned char PID=0x09;//pid indica que este es el PADRE

//	struct Coordsuc vpx; //vpx es VFD  p(x) perpendicular
//VFDdrawLine(1,192,0,0,127);//CX+64,CY-CY,CX-CX,CY+CY-1);

const unsigned char FUERA_DE_GRAFICA='F';//LA LINEA ESta fuera de graficacion


gx.b=*(f+0); gx.m=*(f+1); hx.b=*(f+2); hx.m=*(f+3);
P34.Pm.x=*(f+4);P34.Pm.y=*(f+5);P34.Pn.x=*(f+6);P34.Pn.y=*(f+8);
P1.x=*(f+9);P1.y=*(f+10);
fx.b=*(f+11);fx.m=*(f+12);
P58.Pm.x=*(f+13);P58.Pm.y=*(f+14);P58.Pn.x=*(f+15);P58.Pn.y=*(f+16);
gx.b=*(f+17);gx.m=*(f+18);//hasta aqui no se ha contado esta linea
P67.Pm.x=*(f+19);P67.Pm.y=*(f+20);P67.Pn.x=*(f+21);P67.Pn.y=*(f+22);
hx.b=*(f+23);hx.m=*(f+24);
P910.Pm.x=*(f+25);P910.Pm.x=*(f+26);P910.Pm.x=*(f+27);P910.Pm.x=*(f+28);
m=*(f+29);
px.b=*(f+30);px.m=*(f+31);
Pab.Pm.x=*(f+32);Pab.Pm.y=*(f+33);Pab.Pn.x=*(f+32);Pab.Pn.y=*(f+32);
angi=*(f+33);
A1.b=*(f+34);A1.m=*(f+35);
A2.b=*(f+36);A2.m=*(f+37);
Pcd.Pm.x=*(f+38);Pcd.Pm.y=*(f+39);Pcd.Pn.x=*(f+40);Pcd.Pn.y=*(f+41);


estado2=*inst;
vfx.Pa.x=*(inst+1); vfx.Pa.y=*(inst+2); vfx.Pb.x=*(inst+3); vfx.Pb.y=*(inst+4);
vgx.Pa.x=*(inst+5); vgx.Pa.y=*(inst+6); vgx.Pb.x=*(inst+7); vgx.Pb.y=*(inst+8);
vhx.Pa.x=*(inst+9); vhx.Pa.y=*(inst+10);vhx.Pb.x=*(inst+11);vhx.Pb.y=*(inst+12);
vA1.Pa.x=*(inst+13);vA1.Pa.y=*(inst+14);vA1.Pb.x=*(inst+15);vA1.Pb.y=*(inst+16);
vA2.Pa.x=*(inst+17);vA2.Pa.y=*(inst+18);vA2.Pb.x=*(inst+19);vA2.Pb.y=*(inst+20);
vgx0.Pa.x=*(inst+21); vgx0.Pa.y=*(inst+22); vgx0.Pb.x=*(inst+23); vgx0.Pb.y=*(inst+24);
vhx0.Pa.x=*(inst+25); vhx0.Pa.y=*(inst+26); vhx0.Pb.x=*(inst+27); vhx0.Pb.y=*(inst+28);


switch(estado2){
case 1: if((angulo<0.2)&&(angulo>0)) angulo=0.1;estado2++;break;
case 2: if(angulo==90) return TRUE;	
case 3: P1.x = 96; P1.y = -64;estado2++;break;
case 4: fx=getEcuacion_Recta(P1,angulo);estado2++;break;
case 5: P34=get_Puntos(fx,angulo,CENTRAL,sens);
        estado2++;break;
case 6: P58 = get_Puntos_Perpendiculares_v2(fx,P1,sens);estado2++;break;
case 7: gx=getEcuacion_Recta(P58.Pn,angulo);estado2++;break;
case 8: hx=getEcuacion_Recta(P58.Pm,angulo);estado2++;break;
case 9: if(is_graficable(gx,hx)){dds.b.b.isGraficable0=1;estado2++;}
        else{if(dds.b.b.isGraficable0) estado2=50;//que borre o no las lineas anteriores
             else{dds.b.b.isGraficable0=0;estado2=199;}}
        break;
case 50:if(VFDdrawLine_v5(0, vgx0.Pa.x, vgx0.Pa.y, vgx0.Pb.x, vgx0.Pb.y,100,inst+29,usi,PID))estado2++;break;	 
case 51:if(VFDdrawLine_v5(0, vhx0.Pa.x, vhx0.Pa.y, vhx0.Pb.x, vhx0.Pb.y,100,inst+29,usi,PID))estado2++;break;	 

case 10: P67=get_Puntos(gx,angulo,PARRIBA,sens);estado2++;break;
case 11:P910=get_Puntos(hx,angulo,PABAJO,sens);
	   if(((gx.m>0)&&(gx.b>0))||((hx.m>0)&&((hx.m*(192)+hx.b)<-128))){
	        estado2=199;break;} 
	   estado2++;break;
case 12:m=calculo_pendiente_inversa(angulo);estado2++;break;//directriz de la Altura.
case 13:px=getEcuacion_Recta_Punto_Pendiente(P1,m);estado2++;break;//ec. perpendicular ala directriz
case 14:Pab =busqueda_Puntos_Altura(fx,sens);estado2++;break;//puntos en directriz inversa donde cruza la altura. 
case 15:angi=get_angulo_Perpendicular(m,angulo);estado2++;break;//angulo de la perpendicular
	   
case 16:A1 =getEcuacion_Recta(Pab.Pm,angi);estado2++;break;//ecuacion de tapa Altura
case 17:A2 =getEcuacion_Recta(Pab.Pn,angi);estado2++;break;//get ec de recta de la tapa  Altura,perp  a directriz
	
case 18:Pab.Pm=get_Puntos_de_Interseccion(gx,A1);estado2++;break; //PUNTO interseccion Altura y sensibilidad	
case 19:Pab.Pn=get_Puntos_de_Interseccion(hx,A1);estado2++;break;
case 20:Pcd.Pm=get_Puntos_de_Interseccion(gx,A2);estado2++;break; //PUNTO interseccion Altura y sensibilidad	
case 21:Pcd.Pn=get_Puntos_de_Interseccion(hx,A2);estado2++;break;
	
case 22:vfx = traducirCoordenadas_to_VFD_v2(P34,fx);estado2++;break;//puntos P3 y P4
case 23:vgx = traducirCoordenadas_to_VFD_v2(P67,gx);estado2++;break; 
case 24:vhx = traducirCoordenadas_to_VFD_v2(P910,hx);estado2++;break;
case 25:vA1 = traducirCoordenadas_to_VFD_v2(Pab,A1);estado2++;break;
case 26:vA2 = traducirCoordenadas_to_VFD_v2(Pcd,A2);estado2++;break;
case 27:estado2=100;break;
	   
	   
	   
//SECCION DE DIBUJAR++++++++++++++++++++++++++++++++++++++	   
case 100:if(VFDdrawLine_v5(0, vgx0.Pa.x, vgx0.Pa.y, vgx0.Pb.x, vgx0.Pb.y,100,inst+29,usi,PID))estado2++;break;	 
case 101:if(VFDdrawLine_v5(0, vhx0.Pa.x, vhx0.Pa.y, vhx0.Pb.x, vhx0.Pb.y,100,inst+29,usi,PID))estado2++;break;	 
case 102://estado2++;break;//
         if(VFDdrawLine_v5(1, vfx.Pa.x, vfx.Pa.y, vfx.Pb.x, vfx.Pb.y,100,inst+29,usi,PID))estado2++;break;
case 103:if((gx.m>0)&&(gx.b>0)){estado2++;estado2++;}else{estado2++;}break;//No pintar g(x)
case 104:if(VFDdrawLine_v5(1, vgx.Pa.x,vgx.Pa.y,vgx.Pb.x,vgx.Pb.y,100,inst+29,usi,PID))estado2++;break;
case 105:if((hx.m>0)&&((hx.m*(192)+hx.b)<-128)){estado2++;estado2++;}else{estado2++;}break;
case 106:if(VFDdrawLine_v5(1, vhx.Pa.x,vhx.Pa.y,vhx.Pb.x,vhx.Pb.y,100,inst+29,usi,PID))estado2++;break;
case 107://estado2++;break;//
         if(VFDdrawLine_v5(1, vA1.Pa.x, vA1.Pa.y, vA1.Pb.x, vA1.Pb.y,100,inst+29,usi,PID))estado2++;break;
case 108://estado2++;break;//
	     if(VFDdrawLine_v5(1, vA2.Pa.x, vA2.Pa.y, vA2.Pb.x, vA2.Pb.y,100,inst+29,usi,PID))estado2++;break;
case 109: if(VFDdrawLine_v5(1,0 ,64,192, 64,100,inst+29,usi,PID))estado2++;break;
case 110:if(VFDdrawLine_v5(1,96, 0, 96,127,100,inst+29,usi,PID))estado2++;break;
case 111:vgx0=vgx; vhx0=vhx;estado2=199;break;
case 199:estado2=0;ret=TRUE;break;
default:estado2=1;break;}//fin switch----------------------------	
	
*(inst+ 1)=vfx.Pa.x;  *(inst+2)=vfx.Pa.y;   *(inst+3)=vfx.Pb.x;   *(inst+4)=vfx.Pb.y;
*(inst+ 5)=vgx.Pa.x;  *(inst+6)=vgx.Pa.y;   *(inst+7)=vgx.Pb.x;   *(inst+8)=vgx.Pb.y;
*(inst+ 9)=vhx.Pa.x;  *(inst+10)=vhx.Pa.y;  *(inst+11)=vhx.Pb.x;  *(inst+12)=vhx.Pb.y;
*(inst+13)=vA1.Pa.x;  *(inst+14)=vA1.Pa.y;  *(inst+15)=vA1.Pb.x;  *(inst+16)=vA1.Pb.y;
*(inst+17)=vA2.Pa.x;  *(inst+18)=vA2.Pa.y;  *(inst+19)=vA2.Pb.x;  *(inst+20)=vA2.Pb.y;
*(inst+21)=vgx0.Pa.x; *(inst+22)=vgx0.Pa.y; *(inst+23)=vgx0.Pb.x; *(inst+24)=vgx0.Pb.y;
*(inst+25)=vhx0.Pa.x; *(inst+26)=vhx0.Pa.y; *(inst+27)=vhx0.Pb.x; *(inst+28)=vhx0.Pb.y;
*(f+0)=gx.b;    *(f+1)=gx.m;    *(f+2)=hx.b;    *(f+3)=hx.m;
*(f+4)=P34.Pm.x;*(f+5)=P34.Pm.y;*(f+6)=P34.Pn.x;*(f+8)=P34.Pn.y;
*(f+9)=P1.x;*(f+10)=P1.y;
*(f+11)=fx.b;*(f+12)=fx.m;
*(f+13)=P58.Pm.x;*(f+14)=P58.Pm.y;*(f+15)=P58.Pn.x;*(f+16)=P58.Pn.y;
*(f+17)=gx.b;*(f+18)=gx.m;//hasta aqui no se ha contado esta linea
*(f+19)=P67.Pm.x;*(f+20)=P67.Pm.y;*(f+21)=P67.Pn.x;*(f+22)=P67.Pn.y;
*(f+23)=hx.b;*(f+24)=hx.m;
*(f+25)=P910.Pm.x;*(f+26)=P910.Pm.x;*(f+27)=P910.Pm.x;*(f+28)=P910.Pm.x;
*(f+29)=m;
*(f+30)=px.b;*(f+31)=px.m;
*(f+32)=Pab.Pm.x;*(f+33)=Pab.Pm.y;*(f+32)=Pab.Pn.x;*(f+32)=Pab.Pn.y;
*(f+33)=angi;
*(f+34)=A1.b;*(f+35)=A1.m;
*(f+36)=A2.b;*(f+37)=A2.m;
*(f+38)=Pcd.Pm.x;*(f+39)=Pcd.Pm.y;*(f+40)=Pcd.Pn.x;*(f+41)=Pcd.Pn.y;
     ...
     switch(estado){
    	  case 1:
			     ...
			     if(operacion==SUMAR)
				  ...
				  estado++;break;
    	  case 2:if(getCharsFromUINT_var1(p,*pn,v+1))
    	         	  estado++;
    	         break;
    	  case 3:display5UChars1(posx,posy,p);
    		     estado=0;
    		     ret=TRUE;
    	         break;
    	 default:estado=1;break;}
...
unsigned char *p; //para los valores ascii del numero entero
	         ...
	         if(operacion==SUMAR){
	          ...
	          VFDserial_SendChar(a[1]);
    	      VFDserial_SendChar(a[2]);
    	     // Zoom_init();//Debug mplementar ZOOM ESTE Cambia todas las variables para que se grafiquen ene este nuevo zoom
...
unsigned char *p; //para los valores ascii del numero entero
	 ...
	 if(operacion==SUMAR){
		if(++zoom.Zoom>99)
		 ...
		 //posx=digito2posicion(posx);
	  VFDposicion(POSXDDSNUMS,POSY10);
	  VFDserial_SendChar1(a[1]);
	  VFDserial_SendChar1(a[2]);
	 // Zoom_init();//Debug mplementar ZOOM ESTE Cambia todas las variables para que se grafiquen ene este nuevo zoom
}// fin de display de la variable ZZoom en el DDS-------------------------------------------------------------------------
...
void display_Phase_DDS(unsigned char posx,unsigned char posy,unsigned char *p){
	  ...
	  VFDserial_SendChar(*(p+1));
	  VFDserial_SendChar(*(p+2));
	  VFDserial_SendChar(*(p+3));
	  VFDserial_SendChar(*(p+4));
...
unsigned char displayPhase_var1(unsigned short int posx,unsigned short int posy,unsigned char *inst,unsigned char *inst2){//despliega la variable de phase en la posicion
//unsigned char a[3];
auto unsigned char *p,phase,phasefrac,estado; //para los valores ascii del numero entero
auto unsigned char *p2,ret=0;
p=&menu.subMenu.aux1[*inst][0];
phase=menu.subMenu.aux2[*inst];
estado=menu.subMenu.estado[*inst];
phasefrac=menu.subMenu.aux5[*inst];
    switch(estado){
    	case 1:p2=getCharsFromFloat1(&phasefrac,Deteccion.Phase,inst2);
    	       if(*p2==TRUE){
    	    	      phase=*(p2+1);
    	              estado++;}
    	       break;
    	case 2:getUChar2Chars(p,phase,YES);//obtiene los tres char de un numero char unsigned
    	       if(phasefrac>9)
    	          phasefrac=0;
    	       if((posx>0)&&(posx<6))
    	      	        posx=digito2posicion(posx); //convierte numero de orden de digito en coordenadasx del display 
    	       if(posx==0){__asm(nop);__asm(Halt);}//Debug, error
    	       estado++;
    	       break;
    	case 3:VFDposicion(posx,posy);
               VFDserial_SendChar(menu.subMenu.aux1[*inst][0]);
               VFDserial_SendChar(menu.subMenu.aux1[*inst][1]);
               VFDserial_SendChar(menu.subMenu.aux1[*inst][2]);
               VFDserial_SendChar('.');
               VFDserial_SendChar(phasefrac+0x30);
               estado++;
               break;
    	case 4:Deteccion.Phase=get_Float_from_Phase(phase,phasefrac);
    	       estado=0;ret=TRUE;
               break;	
    	default:estado=1;break;}    
menu.subMenu.aux1[*inst][0]=*p;
menu.subMenu.aux2[*inst]=phase;
menu.subMenu.estado[*inst]=estado;
menu.subMenu.aux5[*inst]=phasefrac;
return ret;
}//display phase var-----------------------------------------------------------------------------
...
unsigned char phase,phasefrac;
 ...
 if((digitopos==1)||(digitopos==2)||(digitopos==3)){
			p=&a[0];
	 getUChar2Chars(p,phase,NEL);
	 if(operacion==SUMAR){
		if(++a[digitopos-1]>9)
			  a[digitopos-1]=0;
		if((digitopos==1)&&(a[digitopos-1]>1))
			  a[digitopos-1]=0;
		phase=a[0]*100+a[1]*10+a[2];}
	  if(operacion==RESTAR){
			if(a[digitopos-1]==0){
				 if(digitopos==1)
					   a[digitopos-1]=1;
...
auto unsigned char ret=0;
   switch(estado6){
	   case 1:if(display_UShort_int(POS_X_SENS,POSY10,inst1,inst2))
		          estado6++;
	           break;
	   case 2:if(display_Float(POS_X_PHASE,POSY10,inst1,inst2)){
		          estado6++;ret=TRUE;}
	           break;
	   default:estado6=1;break;}//fin switch---------------------	
return ret;   
}//fin ---fin display parametros---------------------------------


unsigned char display_UShort_int(unsigned short int x,unsigned short int y,unsigned char *inst1,unsigned char *inst2){
//static unsigned char estado7;
auto unsigned char ret=0;
auto unsigned char *aux,a[5];
auto unsigned char estado;//numero de digitos a desplegar
auto unsigned char cdg;
auto word n;
//const unsigned char  test[]="this is a test ";

   estado=menu.subMenu.estado[*inst1]; //cdg=(estado7&0xF0)>>4;
   cdg=menu.subMenu.count[*inst1];    //estado=estado7&0x0F;
   aux=&menu.subMenu.aux1[*inst1][0];//*(*(a+inst1)+n)
   switch(estado){
	   case 1:getCharsFromUINT_var(aux,Deteccion.Sensibilidad);
	          estado++;break;
	   case 2:vaciar_A2B(aux,&a[0],0,4);estado++;break;
	   case 3:getASCIIfromUCharArray(&a[0],aux,5);estado++;break;
	   case 4:vaciar_A2B(aux,&a[0],0,4);estado++;break;
	   case 5:Formato_USInt(&a[0],aux,5,&cdg);estado++;break;
	   case 6:if(VFDposicion(x,y)) estado++;  break;
	   case 7:if((cdg>5)||(cdg<1))
	               {__asm(nop);__asm(Halt);}
	          estado++;break;
	   case 8:if(VFDserial_SendBlock2(aux,cdg,&n,inst2))
		           estado++;
	   	   	   break;
	   case 9:estado=1;ret=TRUE;break;
	   default:estado=1;break;}//fin switch r
menu.subMenu.estado[*inst1]=estado;//estado7=0;   
menu.subMenu.count[*inst1]=cdg;        //estado7=cdg|estado;   
return ret;	
}//fin display_UShort_int-------------------------------------------
...
auto unsigned char ret=0;
auto unsigned char *aux;
word n; 
   aux=&menu.subMenu.aux1[*inst][0];
   estado=menu.subMenu.estado[*inst];
   switch(estado){
	   case 1:getASCIIFromFloat(aux,Deteccion.phaseDisplay);
	          estado++;
	          break;
	   case 2:if(VFDposicion(x,y))
		             estado++;
	   	   	  break;
	   case 3:if(VFDserial_SendBlock2(aux,5,&n,inst2)){
		            estado=0;ret=TRUE;}
	          break;
	   default:estado=1;break;}
menu.subMenu.estado[*inst]=estado;
...
d17:Ang0=Ang;frac0=frac;veces0=veces;
    VFDposicion(ANG_POSX,ANG_POSY);//POSICION del angulo de vibracion
    ...
    if(num[CENT]>'0'){i=0xAA;j--;//la centena no es cero,un digito a desplegar
    	 VFDserial_SendChar(num[CENT]);}
    if(num[DEC]>'0'){j--;
       	 VFDserial_SendChar(num[DEC]);}
    else{if(i==0xAA){j--;
    	    VFDserial_SendChar('0');}}
    VFDserial_SendChar(num[UNIDAD]);j--;
    VFDserial_SendChar('.');j--;
    if(frac>9){//VFDserial_SendChar(
    	i=0;
    	__asm(Halt);}//debug Error de ingeneiria de Software
    VFDserial_SendChar(frac+0x30);j--;
    if(j>0){
    	VFDserial_SendChar(' ');
        if(j>1)
        	VFDserial_SendChar(' ');}
    j=2;
    VFDposicion(VECES_POSX,VECES_POSY);i=0;
...
unsigned char xInit_VFD(unsigned char *inst){
const unsigned char PID_xInit_VFD1=PID1;//pid de este methodo
auto unsigned char *estado;
//auto unsigned char inst[5];
   if(menu.ret.u.bit.fInit_VFD)
	      return TRUE;
   estado=inst;
  ...
  if(recurso.solicitar(_VFD_,PID_xInit_VFD1)){
   switch(*estado){
	  case 1: (*estado)++;
			  break;
	  case 2: vfd.bits.b.BOX_enable=FALSE;//disable boxes
			  vfd.bits.b.VDF_busy=TRUE;//estamos usando el VFD
			  (*estado)++;
			  break;
	  case 3: if(VFDcommand(0x1BU)) // init display  ESC@= 1BH,40H
				  (*estado)++;
			  break;
	  case 4: if(VFDcommand(0x40U))	 // Setting return to default values
                   (*estado)++;
			  break;
	  case 5:if(FontSizeVFD(FONTSIZE2,inst+1))//TAMAÑO DE fuente 2->8x16
		           (*estado)++;
			  break;
	  case 6:
...
//float f;
	 ...
	 VFDdrawLine(1,1,127-w,192,127-w);}
...
/*VA a monitorear la variable de rechazos y la va a procesar
 * para desplegarla en el portal inicio
 *   +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ **/
unsigned char Display_Monitor_de_Rechazos_VFD(unsigned char *p,unsigned long int *ulint){
auto unsigned char estado10;
const unsigned char SIZE_LONG=10U;
auto unsigned char *n;//n[SIZE_LONG];
//static unsigned short int rechazo1;
static unsigned long int rechazo1,rechazo2;
const unsigned char PID_Mon_Rechazo6=PID6;
auto unsigned char m1,*inst; 
auto unsigned char ret=0;
auto word nn;
  ...
  if((menu.b.b.MenuPendiente)&&(recurso.v.asignado==PID_Mon_Rechazo6)){
  	 if(estado10>8) goto z2w3;
  	 else{recurso.devolver(_VFD_,PID_Mon_Rechazo6);return(TRUE);}}
  		 
    if(menu.b.b.MenuPendiente)
  	     return(TRUE);
  
  if(menu.contexto.Actual==PORTAL_INICIO){
    if(Deteccion.rechazo.NumRech==rechazo2)
    	ret=TRUE;
	else{
		if(recurso.solicitar(_VFD_,PID_Mon_Rechazo6)){  
z2w3:switch(estado10){
		  case 0:estado10=1;m1=0;break;	 
		  case 1:estado10++;
				 break;
		  case 2:rechazo1=Deteccion.rechazo.NumRech;
				 estado10++;
				 break;
		  case 3:convertir_ULInt_to_ASCII(&n[0],rechazo1);
				 estado10++;
				 break;
		  case 4:m1=Formato_ULInt(0,n);	
				 estado10++;	
				 break;
		  case 5:if(VFDposicion(POS_X_CNTA_RECH,POSY12))	
					  estado10++;
				 break;
		  case 6:if(VFDserial_SendBlock1(&n[m1],((unsigned short)(10-m1)),&nn,inst))
						  estado10++;
				 break;
		  case 7:if(delay_us_VFD(1000))
						 estado10++;
				 break; 
		  case 8:ret=TRUE;
				 rechazo2=rechazo1;
				 estado10=0;
				 recurso.devolver(_VFD_,PID_Mon_Rechazo6);
				 break;
		  default:estado10=1;break;}  
         }else ret=TRUE;//recurso no disponible
	}//fin else
  }//fin Portal inicio ---------------------------------------
*p=estado10;
*(p+1)=m1;
*ulint=rechazo1;
return ret;  
}//fin Display_Monitor_de_Rechazos_VFD----------------------------------


unsigned char Display_Monitor_de_Producto_VFD(unsigned char *p,unsigned short int *usi){
auto unsigned char estado11;	
auto unsigned char *n1;//n1[5];
static unsigned short int prod1,prod2;
auto unsigned char a[5];
const unsigned char PID_Mon_Prod7=PID7;
auto unsigned char k;    
auto word nn;
auto unsigned char ret=0;
auto unsigned char *inst;

estado11=*p;
k=*(p+1);
n1=p+2;//5,6,7,8,9 lugarr
prod1=*usi;
inst=p+10;


if((menu.b.b.MenuPendiente)&&(recurso.v.asignado==PID_Mon_Prod7)){
  	 if(estado11>8) goto e3d1;
  	 else{recurso.devolver(_VFD_,PID_Mon_Prod7);return(TRUE);}}
  		 
    if(menu.b.b.MenuPendiente) return(TRUE);

  if(menu.contexto.Actual==PORTAL_INICIO){
   if(Deteccion.prod.NumProd==prod2)
	   ret=TRUE;//no hay cambios 
   else{
	   if(recurso.solicitar(_VFD_,PID_Mon_Prod7)){
	e3d1:switch(estado11){
			  case 0:estado11=1;k=0;break;	
			  case 1:prod1=Deteccion.prod.NumProd;estado11++; break;
			  case 2:k=0;estado11++;break;
			  case 3:estado11++;break;
			  case 4:vfd.bits.b.VDF_busy=TRUE;
					 getCharsFromUINT_var(&n1[0],prod1);
					  estado11++; break;
			  case 5:get_Char2ASCII(&n1[0],&a[0],5);
					 vaciar_A2B(&a[0],&n1[0],0,4); estado11++; break;
			  case 6:Formato_USInt(&n1[0],&a[0],5,&k);
					 vaciar_A2B(&a[0],&n1[0],0,4); estado11++;break;
			  case 7:if(VFDposicion(POS_X_CNTA_PROD,POSY14))estado11++; break;
			  case 8:if(VFDserial_SendBlock1(&n1[0],k,&nn,inst)){estado11++;}
					 break;
			  case 9:if(delay_us_VFD(1000)) estado11++; break;
			  case 10:vfd.bits.b.VDF_busy=FALSE;
					 estado11=0;
					 prod2=prod1;
					 recurso.devolver(_VFD_,PID_Mon_Prod7);
					 ret=TRUE;
					 break;
			  default:estado11=1;break;}//fin switch  
	   }else ret=TRUE; //recurso no disponible
     }//fin else  
   }//fin Portal inicio --------------------------------------------------	
*p=estado11;
...
/*  mEMORIA: uchar:9
 *    float:1   */
unsigned char Display_Monitor_de_Phase_VFD(unsigned char *p){//p+2
auto unsigned char estado16;	
auto unsigned char *n11;//[5];
//static unsigned short int sens1;
static float fase1,fase2;
//auto unsigned char a[5];
auto unsigned char k,*inst;      
unsigned char n2[]={"      "};
auto word nn;
auto unsigned char ret=0;
const unsigned char PID_Mon_Fase5=PID5;
//auto unsigned short int i;
 ...
 if((menu.b.b.MenuPendiente)&&(recurso.v.asignado==PID_Mon_Fase5)){
 	 if(estado16>8) goto er2q;
 	 else{recurso.devolver(_VFD_,PID_Mon_Fase5);return(TRUE);}}
 		 
   if(menu.b.b.MenuPendiente) return(TRUE);
   if(menu.contexto.Actual==PORTAL_INICIO){
	  if(Deteccion.prod.phase==fase2) return TRUE;
	  if(menu.b.b.MenuPendiente) return TRUE;
	  else{
er2q:       switch(estado16){	
			  case 1:estado16++;k=0;break; 
			  case 2:estado16++;break;  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  
			  case 3:fase1=Deteccion.prod.phase;
					 estado16++;break;       
			  case 4:vfd.bits.b.VDF_busy=TRUE;estado16++;break;
			  case 5:estado16++;break;
			  case 6:getASCIIFromFloat(n11,fase1);estado16++;break;		
			  case 7:k=Formato_Phase(n11);estado16++; break;
			  case 8:if(recurso.solicitar(_VFD_,PID_Mon_Fase5))estado16++;break;       
			  case 9:if(VFDposicion(POS_X_PHASE,POSY10)) estado16++;break;
			  case 10:// i=(unsigned short int)(5-k); 
					 if(VFDserial_SendBlock1(n11,5,&nn,inst))estado16++;
					  break;
			  case 11:if(delay_us_VFD(100))estado16++; break;
			  case 12:vfd.bits.b.VDF_busy=FALSE;
					  estado16=0;fase2=fase1;
					  recurso.devolver(_VFD_,PID_Mon_Fase5);
					  ret=TRUE;
					  break;
			  default:estado16=1;break;}}//fin else 
   }//fin Portal inicio --------------------------------------------------	  
*p=estado16;
*(p+1)=k;
return ret;	
}//fin Display_Monitor_de_Phase_VFD-------------------------------------------




//Despliega los Parametros de Portal vInicio
//Solo tiene un Padre y Una instancia
//Memoria: 0+2+
unsigned char xDisplay_Parametros_PORTAL_INICIO_VFD(unsigned char *p,unsigned short int *usi,unsigned long int *ulint){
unsigned char ret=0;	 
  switch(menu.contexto.Actual){
	  case PORTAL_INICIO:if(xDisplay_Barra_Y_Parametros(p,usi,ulint))
		                       ret=TRUE;
		                 break;

	  default:ret=TRUE;break;}	//fin de switch
return ret;  
}//---------------------------------------------------------------------
	

unsigned char Display_Monitor_de_Sensibilidad_VFD(unsigned char *p){
auto unsigned char estado14;	
auto unsigned char *n1;//n1[5];
static unsigned short int sens1,sens2;//memoria siempre activa aunque estemos fuera de cualquier case
auto unsigned char *a;
auto unsigned char k;    
unsigned char n2[]={"      "};
auto word nn;
auto unsigned char ret=0;
auto unsigned char *inst,i;
const unsigned char PID_Mon_Sens8=PID8;
/* memoria mienstras esta activo el case padre*/
estado14=*p;
k=*(p+1);
inst=p+2;//p+3
a=p+4;//4 5 6 7 8 
n1=p+9;//A,B,C,D,E,
  ...
  if((menu.b.b.MenuPendiente)&&(recurso.v.asignado==PID_Mon_Sens8)){
	 if(estado14>8) goto c31x;
	 else{recurso.devolver(_VFD_,PID_Mon_Sens8);return(TRUE);}}
		 
  if(menu.b.b.MenuPendiente)
	     return(TRUE);
  if(menu.contexto.Actual==PORTAL_INICIO){
	if(Deteccion.prod.Sens==sens2)
		ret=TRUE;
	else{
c31x:switch(estado14){  	
	  case 1:estado14++;break; 
	  case 2:estado14++;break;  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  
	  case 3:estado14++;break;  
	         break;       
	  case 4:sens1=Deteccion.prod.Sens;
	  	  	 vfd.bits.b.VDF_busy=TRUE;
	         estado14++;
	         break;
	  case 5:if(getCharsFromUINT_var1(&n1[0],sens1,inst))estado14++;break;
	  case 6:get_Char2ASCII(&n1[0],&a[0],5);
	         vaciar_A2B(&a[0],&n1[0],0,4);
	         estado14++;
	         break;
	  case 7:Formato_USInt(&n1[0],&a[0],5,&k);	
		     vaciar_A2B(&a[0],&n1[0],0,4);
		     clearFormato(&n1[0]);
		     estado14++;
	 	      break;
	  case 8:if(recurso.solicitar(_VFD_,PID_Mon_Sens8))estado14++;break;
	  case 9:if(VFDposicion(POS_X_SENS,POSY10))estado14++;break;
	  case 10:if(delay_us_VFD(100))estado14++;break;
	  case 11:for(i=0;i<5;i++) if(*(n1+i)==0) *(n1+i)=' ';estado14++;break;      
	  case 12:if(VFDserial_SendBlock1(n1,5,&nn,inst))estado14++;break;
	  case 13:if(delay_us_VFD(200))estado14++;break;
	  case 14:vfd.bits.b.VDF_busy=FALSE;
	          estado14=0;
	          sens2=sens1;
	          recurso.devolver(_VFD_,PID_Mon_Sens8);
	          ret=TRUE;
	          break;
	  default:estado14=1;break;} }//fin else 
  }//fin Portal inicio --------------------------------------------------	
*p=estado14;
*(p+1)=k; 
return ret;
...
/* manejador de los cursores del menu de DDS maneja
 * solamente los cursores y los numeros
 *  se ejecuta cuando alguien apreueta un tecla 
 *        */
...
auto unsigned char ret=0;
//auto unsigned char estado3;//si se manipula digito o Fila POSY
const unsigned char PID_Cursores_DDSA=PID10;
const unsigned char Y_ESQUINA_X=192-16;//POSicion Y en cursor esquina
const unsigned char DIGITO=0xD1,ROW=0xDD;//ES digito o ROW
const unsigned char PA_RRIBA=0x80,PA_BAJO=0x40;
//auto unsigned char *inst1;
const unsigned char _20_=20,_30_=30;
auto unsigned char estado4,i,status,init;
static unsigned char(*fcursor)(unsigned char*);
const unsigned char PA_ABAJO=0x80;
const unsigned char _X_240=240;

     if(recurso.v.asignado==PID4) return TRUE;//RAM asignada a otro proceso
     if(menu.contexto.Actual!=PANTALLA_DDS) return TRUE;
     if((recurso.v.asignado==0)&&(menu.b.b.MenuPendiente)) return TRUE;
     
estado4=*inst;
init=*(inst+1);
status=dds.cursor.status;

/*Inizializar y pintar el cursor*/
switch(estado4){//limpiar memoria cada que se aprieta una keypad
	case 1:if(init==0xA1){estado4=8;init=0xAA;}
	       else{if(init!=0xAA){init=0;estado4=2;}
	            else{estado4=8;}}break;	
	case 2:for(i=3;i<SIZE_MEMO_APP;i++) *(inst+i)=0;status&=0x00;
	       estado4++;cursory=0;cursorx=_X_240; break;
	case 3:if(recurso.solicitar(_VFD_,PID10))estado4++;else ret=TRUE;break;
	case 4:if(VFDposicion(cursorx,cursory))estado4++;break;
	case 5:if(VFDserial_SendChar1('>'))estado4++;break;
	case 6:recurso.devolver(_VFD_,PID10);estado4++;break;
	case 7:estado4=90;init=0xA1;break;
	case 8:if(menu.b.b.isCursor){
		     if(status==0xFF){estado4=10;}else{estado4++;}}
		   else{if(recurso.v.asignado==PID_Cursores_DDSA)estado4=90;
		        else{ret=TRUE; }}
	       break;
	case 9:switch(status&0xC0U){
		     case 0x80:fcursor=cursorUpDown;break; //cursor normal up|down
		     case 0xC0:fcursor=digitoUpDown;break;
		     case 0x40:fcursor=digitoRtLeft;break;
		     default:if((status&0x08U)==0x08U)//xxxx 1xxx modo enter 
		    	            fcursor=enterCursorDDS_v2;
		     	 	 else estado4=90;
		             break;}
	       estado4++;break;
    case 10:if(fcursor(inst+2)){status=dds.cursor.status;
                                estado4=90;}break;
    case 11:__asm(nop);__asm(Halt);break;//borrador
	case 90:estado4=0;ret=TRUE;
 	        menu.b.b.isCursor=0;
 	        break;
...
*inst=estado4;
*(inst+1)=init;
dds.cursor.status=status;


return ret;
}//fin de manejador de xDisplay_Cursores_DDS_VFD-------------------------
...
/* Metodo de solo un Padre y una Estancia  
 *  MEMORIA: uchar:1+xRunMenu(1)=2
 * */
...
if(menu.ret.u.bit.fInit_Menu)//Se inizializo Menu?
	  ret=TRUE;
else{	
  estado=*p; 	
  switch(estado){
	  case 1:BorrarDDS=NONE;  //el DDs no se activado el boton de borrar el DDS
	         ...
	         estado++;
	         break;
	  case 2:if(xRun_Menu(p+1))
	              estado++;
	  	     break;
	  case 3:estado=0;
	         vfd.bits.b.finit_Menu=TRUE;//Se inizializo el MENU?
	         vfd.bits.b.BOX_enable=TRUE;//ya se pueden dibujar BOXes
	         menu.ret.u.bit.fInit_Menu=TRUE;
	         vfd.bits.b.VDF_busy=FALSE;
	         break;
	  default:estado=1;break;}
*p=estado;}
return ret;
...
//un Padre y Una instancia
unsigned char xDisplay_Barra_Y_Parametros(unsigned char *p,unsigned short int *usi,unsigned long int *ulint){
auto unsigned char *estado;
...
auto unsigned char ret=0;
    ...
    count=p+1;
	switch(*estado){
		case 1:if(Display_Monitor_de_Phase_VFD(p+30))        (*estado)++;break;
		case 2:if(Display_Monitor_de_Rechazos_VFD(p+2,ulint))(*estado)++;break;
		case 3:if(Display_Monitor_de_Producto_VFD(p+2,usi))  (*estado)++;break;
		case 4:if(Display_Monitor_de_Sensibilidad_VFD(p+15)) (*estado)++;break;
		case 5:ret=TRUE;(*estado)=0;break;
		default:*estado=1; break;}//fin switch--------------------
return ret;
}//fin -------display_Barra_Y_Parametros----------------------------------
