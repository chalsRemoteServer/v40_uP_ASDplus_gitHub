...
//extern void cambio_de_contexto(unsigned char destino);
...
/*case PORTAL_INICIO: cambio_de_contexto(PORTAL_INICIO;//MENU-PADRE:PORTAL_INICIO----------------------
...
void PortaliniciokeyEN(void){
	                         ...
	                         cambio_de_contexto(MENU_INSIGHT);/*a donde voy*/return;}//fin PortaliniciokeyDN--------------------------------------------------------------------------------------

/*fin case PORTAL_INICIO: cambio_de_contexto(PORTAL_INICIO;//MENU-PADRE:PORTAL_INICIO
...
//						menu.contexto.Anterior=AJUSTE_PARAMETRICO_DE_PRODUCTO;
						...
						break;
			case POSY0:
			        	cambio_de_contexto(configModificacionController(PORTAL_INICIO,AJUSTE_PARAMETRICO_DE_PRODUCTO));
			        	break;
...
void AjustedeSistemakeyDN(void){
	 	 if(cursory==POSY0){
	 		 ...
	 		 VFDserial_SendChar('>');return;}
	 	 		 ...
	 	 		 if(cursory<POSY14){
	 	 			 __asm(nop);
	 	 			 switch(menu.contexto.control){ //Autor=3
	 	 				 case 1: 
	 	 				 case 2:return;break;
	 	 				 case 3:
...
void AjustedeSistemakeyEN(void){
	...
	if(cursory==POSY8){ cambio_de_contexto(CONFIGURAR_ENTRADAS_DE_SISTEMA); cursorAnterior=POSY0;return;}
...
unsigned char z[3];
	   	...
	   	if(!menu.contexto.permisos)//passControl[_AUTHOR_]==0)
	   		 p=&key2[0];
	   	else p=&key[0];
	   	...
	   	if( DDSpant->igxc5>0){//esta seleccionado cambiar por numero
				...
				switch(cursory){
	   			...
	   			return;}
	   	switch(cursory){
	   		case POSY2: VFDposicion(cursorx,cursory);
						VFDserial_SendChar1(' ');
						cursory=POSY0;	  
						VFDposicion(POSXESQ235,cursory);
						VFDserial_SendChar1('>');
	   		...
	   		case POSY10:switch(DDSpant->igxc1){
					       case 2:VFDposicion(POSXDDSNUMS,POSY10);
	    				   	      ...
	    				   	      VFDserial_SendChar1(z[1]);
	    				   	      VFDserial_SendChar1(0x01);
					              isEnable_Keypad(WAIT);
                                  break;
	   					   case 1:VFDposicion(POSXDDSNUMS,POSY10);
	   					   	   	  getASCII_from_UChar(zoom.Zoom,&z[0]);
	   					   	   	  procesar_ASCII(&z[0],SUMAR,RIGHT);	
	   						      zoom.Zoom=convert_ASCII_to_char(&z[0]);
	   					   	      VFDserial_SendChar1(z[1]);
	   					   	      VFDserial_SendChar1('>');
	   					   	      VFDserial_SendChar1(z[2]);
	   					   	      VFDserial_SendChar1(0x01);
								  isEnable_Keypad(WAIT);
								  break;
...
unsigned char get_PosY(unsigned char cursory1,unsigned char size){
unsigned char Size=8,Size2=4;//tamaño de los arrays
unsigned char key[] ={POSY0,POSY2,POSY4,POSY6,POSY8,POSY10,POSY12,POSY14,0};
unsigned char key2[]={POSY0,POSY10,POSY12,POSY14,0};
unsigned char ret=0;
switch(cursory1){
	case POSY0: if(size==Size){ret=0;}else{ret=0;}break;
	case POSY2: if(size==Size){ret=1;}else{ret=4;}break;
	case POSY4: if(size==Size){ret=2;}else{ret=4;}break;
	case POSY6: if(size==Size){ret=3;}else{ret=4;}break;
	case POSY8: if(size==Size){ret=4;}else{ret=4;}break;
	case POSY10:if(size==Size){ret=5;}else{ret=1;}break;
	case POSY12:if(size==Size){ret=6;}else{ret=2;}break;
	case POSY14:if(size==Size){ret=7;}else{ret=3;}break;
	default:__asm(nop);__asm(Halt);break;}//fin switch--------
return ret;
}//------------------------------------------------


void PANTALLA_DDSkeyUP_v2(void){
unsigned char key[] ={POSY0,POSY2,POSY4,POSY6,POSY8,POSY10,POSY12,POSY14,0};
unsigned char key2[]={POSY0,POSY10,POSY12,POSY14,0};
//VARIABLES EN SYNCRONIA CON get_PosY
unsigned char Size=8,Size2=4;//tamaño de los arrays
unsigned char *p,*p2;	
auto unsigned char posy,posx,status,size,digito;
const unsigned char PA_RRIBA=0x80,PA_BAJO=0x40;

//memoria sincronizada con xDisplay_Cursores_DDS_VFD
status=dds.cursor.status;//DDSpant->igxc5=0; var sincronizada //control de digitos sensibilidad
  posy=menu.v.aux[3];//posicion del cursor en Y=0x80 pabajo Y=0x40 parriba
  posx=menu.v.aux[4];//posicion del cursor en X=der=0x80; Y=izq.=0x40
		    
  if(menu.contexto.control==SERVICIO){p=&key[0];p2=&Size;size=Size;}//passControl[_AUTHOR_]==0){
  else{p=&key2[0];p2=&Size2;size=Size2;}
  digito=status&0x07;
  menu.b.b.isCursor=1;//IRQ de cursores
  if((status&0x08)==0x08){//status=1110 1xxx
	       status=0xE8|digito;}
  else{
	  switch(status&0xC0U){//1001=80h|20h: cursorUPDN|PABAJO
		case 0x80:
		case 0x00:status=0xA0;//1010
		          status|=digito;
				  break;//1001 xxxx PA_ABAJO,y posy
		default:__asm(nop);__asm(Halt);}}
			
  dds.cursor.status=status;//DDSpant->igxc5=0; var sincronizada //control de digitos sensibilidad
	menu.v.aux[3]=posy ;//DDSpant->igxc1=0;char Poscion a editar
	menu.v.aux[4]=posx;//DDSpant->igxc2=0;}//indica si se ha hecho un cambio en sensibilidd o  phase u otra cosa y se tiene que repintar el DDS	
}//PANTALLA_DDSkeyUP------------------------------------------------------------------------
...
void PANTALLA_DDSkeyDN_v2(void){
unsigned char key[] ={POSY0,POSY2,POSY4,POSY6,POSY8,POSY10,POSY12,POSY14,0};
unsigned char key2[]={POSY0,POSY10,POSY12,POSY14,0};
unsigned char Size=8,Size2=4;//tamaño de los arrays
unsigned char *p,*p2;	
auto unsigned char posy,posx,status,digito;
const unsigned char PA_RRIBA=0x80,PA_BAJO=0x40;
...
//memoria sincronizada con xDisplay_Cursores_DDS_VFD
  ...
  if(menu.contexto.control==SERVICIO){p=&key[0];p2=&Size;}//passControl[_AUTHOR_]==0){
  else{p=&key2[0];p2=&Size2;}
  digito=status&0x07;	
  menu.b.b.isCursor=1;//IRQ de cursores
  if((status&0x08)==0x08){status=0xD8|digito;}//1101 1xxx
  else{
	  switch(status&0xC0U){//1001=80h|10h: cursorUPDN|PABAJO
		case 0x80:
		case 0x40:
		case 0x00:status=0x90;
		          status|=digito;
				  break;//1001 xxxx PA_ABAJO,y posy
		default:__asm(nop);__asm(Halt);}}
...
void PANTALLA_DDSkeyRT_v2(void){
unsigned char key[] ={POSY0,POSY2,POSY4,POSY6,POSY8,POSY10,POSY12,POSY14,0};
unsigned char key2[]={POSY0,POSY10,POSY12,POSY14,0};
unsigned char Size=8,Size2=4;//tamaño de los arrays
unsigned char *p,*p2;	
auto unsigned char posy,posx,status;
const unsigned char PA_RRIBA=0x80,PA_BAJO=0x40;
...
//	  posy=status&0x07;//salvar digitos
	  menu.b.b.isCursor=1;//IRQ de cursores
	  //if((status&0x08)==0x08) status&=0xF7; //xxxx1xxx,Limpiar Enter
		  switch(cursory){//0101=40h|10h: cursorUPDN|RT
			case POSY0:break;
			case POSY2:
			case POSY4:
			case POSY6:
			case POSY10:status=0x50;break;////0110=40h|10h: cursorUPDN|RT
			case POSY8:
			case POSY12:
			case POSY14:break;
			default:__asm(nop);__asm(Halt);}
...
void PANTALLA_DDSkeyLF_v2(void){
unsigned char key[] ={POSY0,POSY2,POSY4,POSY6,POSY8,POSY10,POSY12,POSY14,0};
unsigned char key2[]={POSY0,POSY10,POSY12,POSY14,0};
unsigned char Size=8,Size2=4;//tamaño de los arrays
unsigned char *p,*p2;	
...
posx=menu.v.aux[4];//posicion del cursor en X=der=0x80; Y=izq.=0x40
  ...
  menu.b.b.isCursor=1;//IRQ de cursores
  //if((status&0x08)==0x08) status&=0xF7; //xxxx1xxx,Limpiar Enter
  switch(cursory){//0110=40h|10h: cursorUPDN|RT
	case POSY0:break;
	case POSY2:
	case POSY4:
	case POSY6:
	case POSY10:status=0x60;break;////0110=40h|10h: cursorUPDN|RT
	case POSY8:
	case POSY12:
	case POSY14:break;
	default:__asm(nop);__asm(Halt);}
...
void PANTALLA_DDSkeyEN_v2(void){
unsigned char key[] ={POSY0,POSY2,POSY4,POSY6,POSY8,POSY10,POSY12,POSY14,0};
unsigned char key2[]={POSY0,POSY10,POSY12,POSY14,0};
unsigned char *p;	
auto unsigned char posy,posx,status,digito;
...
//memoria sincronizada con xDisplay_Cursores_DDS_VFD digitoRtLeft cursorUpDown etc
status=dds.cursor.status;//DDSpant->igxc5=0; var sincronizada //control de digitos sensibilidad
  posy=menu.v.aux[3];//posicion del cursor en Y=0x80 pabajo Y=0x40 parriba
  posx=menu.v.aux[4];//posicion del cursor en X=der=0x80; Y=izq.=0x40
  digito=status&0x07;	    
  if(menu.contexto.control==SERVICIO){p=&key[0];}//passControl[_AUTHOR_]==0){
  else p=&key2[0];
  menu.b.b.isCursor=1;//IRQ de cursores
  if((status&0x08)!=0x08){//enter activado
	  switch(status&0xC0){
		  case 0x00:if((status&0x0D)==0x0D){//Activado=Enter|Digito=5
		                   status=0x0F;}break;//apagaer Enter
		  case 0xC0:
		  case 0x80:
		  case 0x40:digito=0;break;//operacion anterior fue CursorUpDn||digitoRtLef
    	  default:__asm(Halt);}}	    
  status|=0x08;//es un comando ENTER
  switch(cursory){
	  case POSY0:cambio_de_contexto(INFO);break;//regresa el que corresponde
	  case POSY2:
	  case POSY6:if(status==0x0F) break;
		         if(digito==0) digito=1;
	  	  	  	 else{if(digito>0){digito++;
	  	  	  	           if(digito>5) digito=0;}}
	             status&=0x08; //0000 x000 clean digito pos
	             status|=digito;//encendemos el digito a controlar
	             break;
	  case POSY4:if(digito==0) digito=1;
	  	  	  	 else{if(digito>0){ digito++;
	  	  	  	          if(digito>4){ digito=0;status=0x08;break;}}}
      	  	  	 status&=0x08; //0000 x000 clean digito pos
      	  	  	 status|=digito;//encendemos el digito a controlar
      	  	  	 break;
	  case  POSY8:if(digito==1)digito=0;else{digito=1;}
	  	  	  	  status&=0x08; //0000 x000 clean digito pos
	  	  	  	  status|=digito;//encendemos el digito a controlar
	              break;
	  case POSY10:if(++digito<5) digito=0;
	  	  	  	  status&=0x08; //xxxx x000 clean digito pos
	  	  	  	  status|=digito;//encendemos el digito a controlar
	  	  	  	  status|=0x08;//es un comando enter
	  	  	  	  break;
	  case POSY12:if(digito==1)digito=0;else{digito=1;}
	  	  	  	  status&=0x08; //xxxx x000 clean digito pos
	  	  	  	  status|=digito;//encendemos el digito a controlar
	  	  	  	  break;
	  case POSY14:status=0xFF;//BORRADO DE pantalla DDS
	  default:break;}
...
if(cursory==POSY2) {cambio_de_contexto(configModificacionController(CONTROL_PASSWORD,MENU_ADMINISTRATIVO));}
if(cursory==POSY4) {cambio_de_contexto(configModificacionController(RELOJ_DE_SISTEMA,MENU_ADMINISTRATIVO));}
if(cursory==POSY6) { return;}
if(cursory==POSY8) {cambio_de_contexto(configModificacionController(ESTABLECER_IDIOMA,MENU_ADMINISTRATIVO));} 
if(cursory==POSY10){cambio_de_contexto(configModificacionController(INIT_MEMO_Y_REST,MENU_ADMINISTRATIVO));}
...
if(cursory==POSY14){rotateControldeUsuariovar();
					...
					configModificado(MENU_ADMINISTRATIVO);
					return;}
if(cursory==POSY0){ cambio_de_contexto(configModificacionController(AJUSTE_DE_SISTEMA,MENU_ADMINISTRATIVO));
					return;}
...
void IDcomunicacioneskeyEN(void){
	...
	if(cursory==POSY0){
		cambio_de_contexto(configModificacionController(AJUSTE_DE_SISTEMA,ID_COMUNICACIONES));
		return;}//SI ACTIVAMOS AQUI EL menu control se ejecuta dos veces
...
void ControlFrecuenciaskeyEN(void){
	...
	if(cursory==POSY0) {MemoEEPROM_DDS(SAVE,CONTROL_DE_FRECUENCIA,RESET_DDS_PWG);
						cambio_de_contexto(AJUSTE_DE_SISTEMA);
						return;}//SI ACTIVAMOS AQUI EL menu control se ejecuta dos veces
...
void MedidasVoltajekeyEN(void){
	if(cursory==POSY2) {cambio_de_contexto(HACER_PRUEBA_DE_RUIDO);  return;}
	if(cursory==POSY4) {cambio_de_contexto(MEDIDAS_DE_VOLTAJE);       return;}
	if(cursory==POSY6) {cambio_de_contexto(AJUSTE_DE_PRODUCTOINFO);   return;}
	if(cursory==POSY8) {cambio_de_contexto(TIEMPO_DE_RECHAZOINFO);    return;}
	if(cursory==POSY10){cambio_de_contexto(AJUSTE_SISTEMA_ENTRADAS);  return;}
	if(cursory==POSY12){cambio_de_contexto(ID_MAQUINA);        return;}
	if(cursory==POSY14){cambio_de_contexto(CUENTA_PRODUCTO);   return;}
	if(cursory==POSY0) {gpoc.igpoc0=0;//resetar valores de voltaje a desplegar
		                cambio_de_contexto(INFORMACION_DE_USUARIO); return;}//SI ACTIVAMOS AQUI EL menu control se ejecuta dos veces                          	        
...
void MedidasVoltajeskeyEN(void){
	...
	if(cursory==POSY0) {gpoc.igpoc0=0;//resetar valores de voltaje a desplegar
		                cambio_de_contexto(PRUEBA); return;}//SI ACTIVAMOS AQUI EL menu control se ejecuta dos veces                          	        
}//fin MedidasVoltajekeyEN--------------------------------------------------
...
void TiempoRechazoInfokeyEN(void){cambio_de_contexto(INFORMACION_DE_USUARIO);}
...
void AjusteSistemaEntradaskeyEN(void){cambio_de_contexto(INFORMACION_DE_USUARIO);}
...
void IDmaquinakeyEN(void){cambio_de_contexto(INFORMACION_DE_USUARIO);}
...
void EstablecerIdiomakeyEN(void){cambio_de_contexto(MENU_ADMINISTRATIVO);}
...
void ControlpasswordkeyEN(void){
  if(cursory==POSY0){
	  ...
	  cambio_de_contexto(MENU_ADMINISTRATIVO);
    return;}
  if(cursorx==POSX9CNTRLPASSW){
	   switch(cursory){
		   ...
		   case POSY12:rotateContrasenaVars(POSY12);
					   displayControlContrasenaVal(4);
				 	   configModificado(CONTROL_PASSWORD);
				 	   return;break;
		   case POSY0: cambio_de_contexto(configModificacionController(MENU_ADMINISTRATIVO,CONTROL_PASSWORD));	                
					   return;break;
		   default:return;break;}
...
//	   menu.contexto.Anterior=CONTROL_PASSWORD;
	   configModificado(CONTROL_PASSWORD);
	   cambio_de_contexto(TEXT_PROCESSOR);
...
void RelojSistemakeyEN(void){
	...
	if(cursory==POSY0){
		...
		cambio_de_contexto(configModificacionController(MENU_ADMINISTRATIVO,RELOJ_DE_SISTEMA));}
...
void IngenieriaGracidakeyEN(void){
	if(cursory==POSY2){cambio_de_contexto(GANANCIA_);  return;}
    if(cursory==POSY4){cambio_de_contexto(FILTRO_);return;}
	if(cursory==POSY6){  return;}
	if(cursory==POSY8){  return;}
	if(cursory==POSY10){cambio_de_contexto(PARTE_11); return;}
	if(cursory==POSY12){return;}
	if(cursory==POSY14){cambio_de_contexto(INGENIERIA_GRACIDA2);return;}
	if((cursory==POSY0)&&(cursorx==POSX0)){
						cambio_de_contexto(TIPO_DE_MAQUINA);return;   }
	if((cursory==POSY0)&&(cursorx==POSXESQ235)){
						cambio_de_contexto(AJUSTE_DE_SISTEMA);return;}
...
void TipodeMaquinakeyEN(void){
	if(cursory==POSY0){ cambio_de_contexto(INGENIERIA_GRACIDA);}
	switch(cursory){
    ...
    cambio_de_contexto(configModificacionController(INGENIERIA_GRACIDA,TIPO_DE_MAQUINA));//si se modificaron variables en un contexto mandamos el contexto de estas seguro?
...
void MenuGananciakeyEN(void){
	...
	if(cursory==POSY4){ 
		...
		return;}
	if(cursory==POSY0){
		cambio_de_contexto(configModificacionController(INGENIERIA_GRACIDA,GANANCIA_));
		return;}                              
...
void MenuFiltrokeyEN(void){if(cursory==POSY2){
    ...
    return;}
if(cursory==POSY0){
	cambio_de_contexto(configModificacionController(INGENIERIA_GRACIDA,FILTRO_));
	return;}
return;//return del keyEN 
}
...
void IngenieriaGracida2keyEN(void){
	switch(cursory){
		...
		case POSY10:ReporteProductoVarRotate();
					...
					configModificado(INGENIERIA_GRACIDA2);break;
		case POSY12:cambio_de_contexto(configModificacionController(INGENIERIA_GRACIDA2,PRUEBA));
				    break;/* Prueba */
		case POSY14: /*  Auto set-up sequence */ break;
		case POSY0: //INGRIA-GRACD2=16h  INGERIA-GRACIDA:A9
		           cambio_de_contexto(configModificacionController(INGENIERIA_GRACIDA,INGENIERIA_GRACIDA2));
	 	           break;
		default:break;}
...
switch(cursory){
	...
	case POSY12:cambio_de_contexto(configModificacionController(AJUSTES_AVANZADOS,AJUSTE_TIEMPO_RECHAZO));
	            break;
	case POSY14: cambio_de_contexto(FRECUENCIA_SELECT);break; 
	case POSY0:phase=getCharsFromFloat(&phasefrac,Deteccion.Phase);
		       if((arg5==CAMBIAR)||(arg6==CAMBIAR))
			   ...
			   if(arg1==phase)
					configModificado(AJUSTES_AVANZADOS);
			   if((menu.contexto.Anterior==AJUSTE_DE_PRODUCTO_MODE)&&
...
void FrecuenciaSelectkeyEN(void){
	...
	if(cursory==POSY14){ return;}
	if(cursory==POSY0) {
		cambio_de_contexto(configModificacionController(AJUSTES_AVANZADOS,FRECUENCIA_SELECT));
	  	return;}
...
void PruebaMenukeyEN(void){
	if(cursory==POSY2) {cambio_de_contexto(I2CMENU);return;}
	if(cursory==POSY4) {cambio_de_contexto(I2CEEPROMREAD);return;}
	if(cursory==POSY6) {cambio_de_contexto(I2CNVRAMDATA);return;}
	if(cursory==POSY8) {cambio_de_contexto(SPI_MENU);return;}
	if(cursory==POSY10){cambio_de_contexto(PRUEBAS_SALIDAS);return;}
	if(cursory==POSY12){cambio_de_contexto(PRUEBAS_ENTRADAS);return;}
	if(cursory==POSY14){cambio_de_contexto(MEDIDAS_DE_VOLTAJES); return;}
	if(cursory==POSY0) {cambio_de_contexto(INGENIERIA_GRACIDA2);return;}
}//FIN PruebaMenukeyEN---------------------------------------------
...
void I2CMenukeyEN(void){
	...
	if(cursory==POSY4) {prueba2I2C();/*cambio_de_contexto(*//*MenuControl();*/return;}
	...
	if(cursory==POSY0){cambio_de_contexto(PRUEBA);
                       return;}}
...
void I2CeepromDatakeyEN(void){
	...
	if(cursory==POSY4) {/*cambio_de_contexto(*//*MenuControl();*/return;}
...
void SPIMenukeyEN(void){
	...
	if(cursory==POSY4) {cambio_de_contexto(DISPLAY_ADC);return;}
	if(cursory==POSY6) {/*cambio_de_contexto(;*/return;}
...
unsigned char b[6]={" "};  
 switch(cursory){
	...
	case POSY14:rotate_frecuencySelect_var();
				...
				return;break;
	case POSY0:
		cambio_de_contexto(configModificacionController(AJUSTES_AVANZADOS,PARAMETROS_DE_DETECCION));
//		menu.b.b.semMenuPendiente=TRUE; //habilitamos escape deL Menu por sistema Operativo
//		displayBoxDestructor(ACTIVADO);//destruir y parar la barra de deteccion.
...
void ADCsValueskeyEN(void){
	if(cursory==POSY2) {/*cambio_de_contexto(*/return;}
	if(cursory==POSY4) {/*ontextoActual=;*/return;}
	if(cursory==POSY6) {/*cambio_de_contexto(;*/return;}
	if(cursory==POSY8) {/*cambio_de_contexto(;*/return;}
	if(cursory==POSY10){/*cambio_de_contexto(;*/return;}
	if(cursory==POSY12){/*cambio_de_contexto(*/return;}
	if(cursory==POSY14){/*cambio_de_contexto(;*/return;}
	if(cursory==POSY0){	displayADCvalores=DESACTIVADO;
						cambio_de_contexto(SPI_MENU);
						/*MenuControl();*/return;}
}// FIN ADCsValueskeyEN----------------------------------------
...
void SeleccionarProductokeyEN(void){
	if(cursory==POSY0){//nos salimos no seleccionamos nada
		cambio_de_contexto(AJUSTE_DE_PRODUCTO);//ir al Padre
		 return;} 
...
//		cambio_de_contexto(PORTAL_INICIO);
		...
		//cambio_de_contexto(configModificacionController(AJUSTE_DE_PRODUCTO,SELECCIONAR_PRODUCTO));
        cambio_de_contexto(ESTAS_SEGURO);}
	else 
...
if((arg1==ERROR_PASS)&&(menu.contexto.Actual==CONTROL_PASSWORD)){//hubo password invalida????
      arg1=0;	 
			...
			switch(menu.contexto.control){
				...
				case NOMBRE_PRODUCTO:
					 switch(menu.contexto.padre){
					   case NUEVO_PRODUCTO:swapArrays(&name[0],&Text[0],NOMBRE_PRODUCTO_SIZE);//el nuevo nombre a name y el viejo nombre a text para guardarlo en caso de error o cambio de par
						                   cambio_de_contexto(AJUSTES_AVANZADOS);
					                       break;
				...
				case SUPERVISOR:menu.contexto.control=validatePassword1(&Text[0]);
				                menu.b.b.MenuPendiente=TRUE;//ejecuta el cambio de contexto
				                break;
				default:break;}}
       ...
       if(cursorx==POSX_TEXT_PROCS_X){//NO ACEPTAMOS EL NOMBRE
		   switch(menu.contexto.control){
			    ...
			    case CONTROL_PASSWORD:textProc->igxc0=0;//iniciamos desplegando mayuculas
			    	                  ...
			    	                  cambio_de_contexto(CONTROL_PASSWORD);
			                          return;break;
			    case SUPERVISOR:cambio_de_contexto(menu.contexto.padre);
			    	            break;
			   default:return; break;}
...
void Parte11MenukeyEN(void){
	...
	if(cursory==POSY14){/*cambio_de_contexto(*/return;}
	if(cursory==POSY0){
//  	                 cambio_de_contexto(configModificacionController(Parte11.menuPadre,Parte11.menuNodo);
  	               return;}
...
//		 menu.b.b.semMenuPendiente=TRUE;//semaforo de Menu escape
//         displayBoxDestructor(ACTIVADO);//destruir y parar la barra de deteccion.
         ...
         cambio_de_contexto(PORTAL_INICIO);//configModificacionController(AjusteVibracion.menuPadre,AjusteVibracion.menuNodo); 
...
void AjusteProductoModekeyEN(void){
  switch(cursory){
	case POSY0:switch(menu.contexto.Modificado){
			      case NUEVO_PRODUCTO:cambio_de_contexto(ESTAS_SEGURO2);break;
			      case AJUSTE_PRODUCTO_SEL:
			      default:switch(menu.contexto.Anterior){
			    	        case NUEVO_PRODUCTO:cambio_de_contexto(PORTAL_INICIO);break;
			    	        case AJUSTE_DE_PRODUCTO:if(menu.contexto.control==AJUSTE_DE_PRODUCTO){
			    	        	                       cambio_de_contexto(PORTAL_INICIO);}
			    	        	                    break;//fin case ajuste prod
			    	        default:break;}//din switch contexto anterior default
			              break;}
               break;
    ...
    case POSY4://Ajuste Manual
    	switch(menu.contexto.Anterior){
    		case NUEVO_PRODUCTO:saveNewProduct(arg0);//guardamos el producto nombre y sus variables estandar con el iProduct=arg0
    							cambio_de_contexto(AJUSTES_AVANZADOS);//trae el iProduct desde Select Espacio Nuevo Product
                                break;
    		case AJUSTE_PRODUCTO_SEL: //cambio_de_contexto(AJUSTES_AVANZADOS
...
//unsigned char i;
	if(cursory==POSY0){//nos salimos no seleccionamos nada
	    cambio_de_contexto(AJUSTE_DE_PRODUCTO);
	    return;} 
		arg0=getiProducSelected(cursory);//get the product index selected
		...
		cambio_de_contexto(configModificacionController(AJUSTE_DE_PRODUCTO,BORRAR_PRODUCTO));
...
void CopyProductokeyEN(void){
    ...
    cambio_de_contexto(PASTE_PRODUCTO);
...
void AjusteProductoSelectkeyEN(void){
	if(cursory==POSY0){//nos salimos no seleccionamos nada
		...
		if(validarName(arg0)){//el seleccionado es vacio? es valido
			...
			cambio_de_contexto(AJUSTE_DE_PRODUCTO_MODE);}
		else 
			__asm(Halt);//debug error de diseño de spftware
...
void PasteProductkeyEN(void){	
	 ...
	 cambio_de_contexto(configModificacionController(COPY_PRODUCTO,PASTE_PRODUCTO));
...
const POSICION_X=POSXESQ235;	
  switch(cursory){
	  case POSY0: VFDposicion(POSICION_X,POSY0);
	  	  	  	  VFDserial_SendChar1(' ');
	  	  	  	  VFDserial_SendChar1('x');
	  	  	      VFDserial_SendChar1(0x01);
	  	  	      VFDserial_SendChar1(0x02);
...
void AjusteTiempoRechazokeyEN(void){
   ...
   switch(cursory){
	   case POSY0:cambio_de_contexto(configModificacionController(AJUSTE_TIEMPO_RECHAZO,AJUSTES_AVANZADOS));
...
void EntradaTacokeyEN(void){
	switch(cursory){
	  case POSY0:cambio_de_contexto(CONFIGURAR_ENTRADAS_DE_SISTEMA);
...
unsigned char sensor_valor,m;
  ...
  sensor_valor=*sensor;
  switch(cursory){
			  case POSY0:cambio_de_contexto(CONFIGURAR_ENTRADAS_DE_SISTEMA);break;
    ...
    case BORRAR_PRODUCTO:
    	   ...
    	   if(cursorx==POSXSI){
               ...
               if(arg1==BUSY){//se borro el seleccionado?
                    ...
                    cambio_de_contexto(PORTAL_INICIO);}
               else cambio_de_contexto(AJUSTE_DE_PRODUCTO);
               break;}
    case PASTE_PRODUCTO:
    		if(cursorx==POSX0){
    		...
    		if(cursorx==POSXSI){
    			...
    			cambio_de_contexto(AJUSTE_DE_PRODUCTO);//cambiar el contexto al ajuste de producto
    			break;}
    		break;
    ...
    case AJUSTES_AVANZADOS:
		   if((menu.contexto.Anterior==NUEVO_PRODUCTO)&&
			  ...
			  (menu.contexto.Modificado==AJUSTES_AVANZADOS)){
    		    if(cursorx==POSX0){//No opcion No
    		      safe_things_EstaSeguro2keyEN();
    		   	  cambio_de_contexto(AJUSTE_PARAMETRICO_DE_PRODUCTO);}
    		    else{if(cursorx==POSXSI){
    		       	   MemoEEPROM(SAVE,menu.contexto.Modificado);
    		           cambio_de_contexto(AJUSTE_PARAMETRICO_DE_PRODUCTO);}}}
			   else{
				   if(cursorx==POSX0){
						...
						cambio_de_contexto(AJUSTE_PARAMETRICO_DE_PRODUCTO);}
						   ...
						   MemoEEPROM(SAVE,menu.contexto.Modificado);
						   cambio_de_contexto(AJUSTE_PARAMETRICO_DE_PRODUCTO);}}}
    	   break;//break de ajustess avanzados
...
void safe_things_EstaSeguro2keyEN(void){
unsigned char i,phasefrac,phase;	
...
}//finsafe_things_EstaSeguro2keyEN-------------------------
...
/* SISTEMA OPERATIVO MANEJO DE KEYS *******************************/
/******************************************************************+++*/
/******************************************************************+++*/
/******************************************************************+++*/
/******************************************************************+++*/
